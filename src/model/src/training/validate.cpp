...
/**
 * @brief validate the first block of transformer
 ...
 */
void model::validateBlock(const std::string& txtFileLocation) 
{
    ...
    for(int i = 0; i < numberOfLines; i++)
    {
        ...
        for(int i = 0; i < oddSentence.size(); i++) {
            ...
            {
                #ifdef USE_CUDA
                    this->T.cuValidate(promptEmbeddings, responseEmbeddings, responseTokens);
                #elif USE_OPENCL
                    this->T.clValidate(promptEmbeddings, responseEmbeddings, responseTokens);
                #elif USE_CPU
                    this->T.validate(promptEmbeddings, responseEmbeddings, responseTokens);
                #endif
            }
            else {
                throw std::runtime_error("LOCAL CONTEXT LIMIT of REACHED");
 ...
 */
void model::validateModel(const std::string& txtFileLocation) 
{
    // basic checks for file
    if (txtFileLocation.empty()) {
        throw std::runtime_error("Training data file path cannot be empty!");
