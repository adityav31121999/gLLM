 ...
 */
void randomweights(std::vector<std::vector<double>> weights) {
    // Randomly intialize weights
    for(size_t i = 0; i < weights.size(); i++) {
        std::transform(weights[i].begin(), weights[i].end(), weights[i].begin(), [i](double x) {
 ...
 */
void jumbledwbs(std::vector<std::vector<double>> weights) {
 ...
 */
void ijbasedwbs(std::vector<std::vector<double>> weights) {
    // Randomly initialize weights
    for(int i = 0; i < weights.size(); i++) {
        std::transform(weights[i].begin(), weights[i].end(), weights[i].begin(), 
                       [i](double x) mutable { 
                        return static_cast<double>(static_cast<int>(((i+1)*(x+1)) * rand()) % static_cast<int>(i + x)); 
                       });
    }
}
 ...
 */
void Random(std::vector<std::vector<double>> weights) {
