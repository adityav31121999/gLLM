...
/**
 * @brief Host function to compute matrix-vector product using the first 'dot' CUDA kernel.
 *        Calculates result = matrix * vec. Accepts matrix as std::vector<std::vector<float>>.
 * @param h_vec Host input vector.
 * @param h_matrix_2d Host input matrix (2D vector).
 * @return Resulting host vector.
 ...
 */
std::vector<float> host_dot(
    const std::vector<float>& h_vec,
    const std::vector<std::vector<float>>& h_matrix_2d)
{
    // --- Validation ---
    if (h_matrix_2d.empty()) {
        throw std::invalid_argument("Input matrix cannot be empty.");
    }
    int matrixRow = h_matrix_2d.size();
    if (matrixRow <= 0) {
         throw std::invalid_argument("Matrix must have at least one row.");
    }
    int matrixColumn = h_matrix_2d[0].size();
    if (matrixColumn <= 0) {
        throw std::invalid_argument("Matrix must have at least one column.");
    }

    // Check for ragged matrix (all rows must have the same number of columns)
    for (size_t i = 1; i < h_matrix_2d.size(); ++i) {
        if (h_matrix_2d[i].size() != static_cast<size_t>(matrixColumn)) {
            throw std::invalid_argument("Input matrix is ragged (rows have different lengths).");
        }
    }
    ...
    // --- Flatten the 2D Host Matrix ---
    std::vector<float> h_matrix_flat(static_cast<size_t>(matrixRow) * matrixColumn);
    for (int i = 0; i < matrixRow; ++i) {
        for (int j = 0; j < matrixColumn; ++j) {
            h_matrix_flat[static_cast<size_t>(i) * matrixColumn + j] = h_matrix_2d[i][j];
        }
    }

    // --- GPU Resource Allocation ---
    float *d_vec = nullptr, *d_matrix = nullptr, *d_vecOutput = nullptr;
    std::vector<float> h_vecOutput(matrixRow); // Allocate host result vector

    try {
        size_t vec_size_bytes = static_cast<size_t>(veclength) * sizeof(float);
        size_t matrix_size_bytes = h_matrix_flat.size() * sizeof(float);
        ...
        // Copy the FLATTENED matrix data
        CHECK_CUDA_ERROR(cudaMemcpy(d_vec, h_vec.data(), vec_size_bytes, cudaMemcpyHostToDevice));
        CHECK_CUDA_ERROR(cudaMemcpy(d_matrix, h_matrix_flat.data(), matrix_size_bytes, cudaMemcpyHostToDevice));
        ...
        // --- Kernel Launch ---
        // Kernel still expects flattened data and dimensions
        ...
        CHECK_CUDA_ERROR(cudaMemcpy(h_vecOutput.data(), d_vecOutput, output_vec_size_bytes, cudaMemcpyDeviceToHost));
 ...
 *        Accepts matrix as std::vector<std::vector<float>>.
 * @param h_vec1 Host input vector 1 (T1).
 * @param h_matrix_2d Host input matrix M (2D vector, must be square N x N).
    ...
    const std::vector<float>& h_vec2)
{
    ...
    if (matrixRow != matrixColumn) {
        throw std::invalid_argument("Input matrix must be square for vector-matrix-vector product.");
    }
    int N = matrixRow; // The common dimension

    // Check for ragged matrix
    for (size_t i = 1; i < h_matrix_2d.size(); ++i) {
        if (h_matrix_2d[i].size() != static_cast<size_t>(N)) {
            throw std::invalid_argument("Input matrix is ragged (rows have different lengths).");
        }
    }
    ...
    try {
        ...
        size_t matrix_size_bytes = h_matrix_flat.size() * sizeof(float);
        size_t scalar_size_bytes = sizeof(float);
        ...
        // --- Kernel Launch ---
        // Kernel still expects flattened data and dimensions N, N, N, N
