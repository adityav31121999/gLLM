...
/**
 * @brief CUDA Kernel: Calculates the determinant of a 2x2 matrix.
 * @param flattenedMatrix Pointer to the 2x2 matrix elements in GPU memory (row-major).
 * @param d_result Pointer to a float in GPU memory where the result will be stored.
 * @param n Dimension of the matrix (must be 2).
 */
__global__ void det2(float* flattenedMatrix, float* d_result, int n) {
    // This kernel is intended to run with a single thread.
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        if (n != 2) {
            // Basic error check within kernel (optional, better handled on host)
            // Consider using printf or setting an error flag if needed.
            return;
        }
...
/**
 * @brief CUDA Kernel: Calculates the determinant of a 3x3 matrix using Sarrus rule.
 * @param flattenedMatrix Pointer to the 3x3 matrix elements in GPU memory (row-major).
 * @param d_result Pointer to a float in GPU memory where the result will be stored.
 * @param n Dimension of the matrix (must be 3).
 */
__global__ void det3(float* flattenedMatrix, float* d_result, int n) {
    // This kernel is intended to run with a single thread.
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        if (n != 3) {
        ...
        // Using the formula from det.cpp (which seems like cofactor expansion on row 0)
        // float det = a00 * (a11 * a22 - a12 * a21)
        //           - a01 * (a10 * a22 - a12 * a20) // Corrected C++ formula had error here
        //           + a02 * (a10 * a21 - a11 * a20);

        // Using the exact (but slightly unusual expansion) formula from det.cpp:
         *d_result = a00*(a11 * a22 - a12 * a21)
                    -a01*(a00 * a12 - a02 * a10) // This part of the C++ formula is incorrect for standard det3
                    +a02*(a00 * a11 - a01 * a10); // This part is also incorrect

        // Let's implement the *standard* Sarrus / cofactor expansion for det3:
        float det_correct = a00 * (a11 * a22 - a12 * a21)
                          - a01 * (a10 * a22 - a12 * a20)
                          + a02 * (a10 * a21 - a11 * a20);

        *d_result = det_correct; // Use the standard correct formula
    }
}


/**
 * @brief CUDA Kernel: Calculates the determinant of a 4x4 matrix using the specific formula from det.cpp.
 * @param flattenedMatrix Pointer to the 4x4 matrix elements in GPU memory (row-major).
 * @param d_result Pointer to a float in GPU memory where the result will be stored.
 * @param n Dimension of the matrix (must be 4).
 */
__global__ void det4(float* flattenedMatrix, float* d_result, int n) {
     // This kernel is intended to run with a single thread.
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        if (n != 4) {
            return; // Basic error check
        ...
        // Direct translation of the formula in det.cpp (Laplace expansion or similar)
...
/**
 * @brief CUDA Kernel: Calculate the determinant of an n x n matrix using Gaussian elimination (row reduction).
 *        NOTE: This kernel modifies the input matrix `flattenedMatrix` IN PLACE.
 *        NOTE: This single-threaded kernel is unlikely to be faster than CPU for this task.
 * @param flattenedMatrix Pointer to the n x n matrix elements in GPU memory (row-major). Will be modified.
 * @param d_result Pointer to a float in GPU memory where the result will be stored.
 * @param n Dimension of the square matrix.
 */
__global__ void detn(float* flattenedMatrix, float* d_result, int n) {
    // This kernel is intended to run with a single thread.
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        if (n <= 0) {
        ...
        for (int k = 0; k < n; k++) { // Iterate through pivot columns/rows
            ...
            int pivot_row = k;
            float max_val = abs(flattenedMatrix[IDX(k, k, n)]);
            for (int i = k + 1; i < n; i++) {
                if (abs(flattenedMatrix[IDX(i, k, n)]) > max_val) {
                    max_val = abs(flattenedMatrix[IDX(i, k, n)]);
                    pivot_row = i;
                }
            }
        ...
        // Using powf is also valid: det *= powf(-1.0f, row_swap_counter);
...
/**
 * @brief CUDA Kernel: Calculates the trace of a square matrix (sum of diagonal elements).
 * @param flattenedMatrix Pointer to the n x n matrix elements in GPU memory (row-major).
 * @param d_result Pointer to a float in GPU memory where the result will be stored.
 * @param n Dimension of the square matrix.
 */
__global__ void trace(float* flattenedMatrix, float* d_result, int n) {
    // This kernel is intended to run with a single thread.
     if (threadIdx.x == 0 && blockIdx.x == 0) {
        if (n <= 0) {
             *d_result = 0.0f; // Or some error indicator
