 ...
 * @param vec Pointer to the input vector 'a' in global GPU memory.
 * @param matrix Pointer to the input matrix 'b' in global GPU memory (row-major).
 * @param vecOutput Pointer to the output vector 'c' in global GPU memory.
...
/**
 * @brief OpenCL Kernel: Computes the scalar dot product: result = vec1 * matrix * vec2_transpose.
 *        NOTE: Requires launch with a SINGLE workgroup and shared memory allocation.
 *              Requires workgroup size >= N (dimension), ideally power of 2.
 * @param vec1 Pointer to input vector T1 in global GPU memory.
 * @param matrix Pointer to input matrix M in global GPU memory (row-major, N x N).
 * @param vec2 Pointer to input vector T2 in global GPU memory.
 * @param d_scalarOutput Pointer to a single float in global GPU memory for the output scalar result.
 ...
 */
__kernel void dot_vector_matrix_vector(
    __global float* vec1,
    __global float* matrix,
    __global float* vec2,
    __global float* d_scalarOutput,
    int vec1length,
    int matrixRow,
    int matrixColumn,
    int vec2length) {
