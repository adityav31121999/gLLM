 ...
 * @param input_mat Input matrix object.
 * @return Additive inverse matrix (-input_mat) as a new mat object.
...
mat host_additive_inverse(const mat& input_mat) {
    ...
    int rows = input_mat.row;
    int cols = input_mat.col;
    size_t num_elements = static_cast<size_t>(rows) * cols;
    // size_t matrix_size_bytes = num_elements * sizeof(float);

    float* d_matrix = nullptr;
    float* d_result = nullptr;
    // Create a new mat object for the result (allocates temp file and maps)
    mat result_mat(rows, cols);
    ...
    return result_mat; // Return the new mat object
}
...
mat host_inverse_gauss_jordan(const mat& input_mat) {
    ...
    // Create a new mat object for the result
    mat result_mat(n);
    if (!result_mat.mapped_data) { throw std::runtime_error("Failed to create result matrix."); }
