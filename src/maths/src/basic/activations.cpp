...
float ReLU(const float& x) {
    ...
    return std::max(0.0f, x); // Use float literal 0.0f
}
...
std::vector<std::vector<float>> ReLU(const std::vector<std::vector<float>>& x, int t) { // Pass t by value
    if (x.empty() || t <= 0) return {};
    size_t num_rows = std::min((size_t)t, x.size());
    std::vector<std::vector<float>> result;
    result.reserve(num_rows);

    for (size_t i = 0; i < num_rows; ++i) {
        size_t num_cols = std::min((size_t)t, x[i].size());
 ...
 */
std::vector<std::vector<float>> ReLUder(const std::vector<std::vector<float>>& x, int t) { // Pass t by value
    if (x.empty() || t <= 0) return {};
    size_t num_rows = std::min((size_t)t, x.size());
    std::vector<std::vector<float>> result;
    result.reserve(num_rows);
...
std::vector<std::vector<float>> LOTA(const std::vector<std::vector<float>>& y, int t, bool attentionType) { // Pass t and attentionType by value
    ...
    size_t max_rows = std::min((size_t)t, x.size());
    for (size_t i = 0; i < max_rows; ++i) {
        size_t limit_j = attentionType ? (i + 1) : (size_t)t;
        limit_j = std::min(limit_j, x[i].size()); // Boundary check cols
...
std::vector<std::vector<float>> LOTAder(const std::vector<std::vector<float>>& y, int t, bool attentionType) { // Pass t and attentionType by value
    ...
    float min_val = std::numeric_limits<float>::max();
    bool found_value = false;
    size_t max_rows = std::min((size_t)t, y.size()); // Use y for reading min
    for (size_t i = 0; i < max_rows; ++i) {
        size_t limit_j = attentionType ? (i + 1) : (size_t)t;
        limit_j = std::min(limit_j, y[i].size()); // Boundary check cols
        for (size_t j = 0; j < limit_j; ++j) {
            min_val = std::min(min_val, y[i][j]);
            found_value = true;
        }
    }
