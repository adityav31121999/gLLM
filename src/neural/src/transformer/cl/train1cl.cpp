...
void transformer::clTrain(std::vector<std::vector<float>>& sentence, std::vector<std::string>& rString)
{
    ...
    try {
        ...
        this->blockCount = 1, this->promptCount = currentTokenCount;
        ...
        for (size_t i = 1; i < sentence.size(); ++i) {
            ...
            while (j <= this->epochs) {
                ...
                t[0].clAdamUpdate(l, t_step_adam, beta1, beta2, epsilon, learning);
                // update learning
                if(current_error < prev_Error && current_error != 0.0f) {
                    learning = 1.05 * learning;
                }
                else {
                    learning = 1.25 * learning;
                }

                std::cout << "Adam Update Performed" << std::endl;
                t_step_adam++;
                // learning = learnby;
                totalLearning += learning;
                prev_Error = current_error;
