    ...
    try {
        ...
        while (j <= this->epochs) // Use member variable epochs
        {
            ...
            if(current_block_idx == 1) {
                ...
                clBackward(expected, current_block_idx, clip_norm);
                // t[current_block_idx-1].serialise(t[current_block_idx-1].blockFilePath);
            }
            // t[0].clAdamUpdate(l, t_step_adam, beta1, beta2, epsilon, learning);
            t_step_adam++;
            totalLearning += learning;
...
void transformer::clTrain(std::vector<std::vector<float>>& sentence, std::vector<std::string>& rString)
{
    ...
    try {
        ...
        // Flatten and copy embeddings table
        std::vector<float> flat_embeddings = ::flatten(this->embeddings);           // embedding
        std::vector<float> flat_deEmbeddings = ::flatten(this->deEmbeddings);       // embedding
        d_embeddings = cl::Buffer(this->clcontext.context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, embeddingsBytes, flat_embeddings.data(), &cl_err); CL_CHECK(cl_err);
        ...
        for (size_t i = 1; i < sentence.size(); ++i) { // i is the index of the TARGET token in 'sentence'
            ...
            while (j < initial_epochs_per_token_limit) { // Use initial limit for fixed epochs per token
                ...
                if (this->learning <= LEARNING_MIN && this->lr_patience_counter >= this->LR_PATIENCE_CONFIG) {
                    std::cout << "Learning rate at minimum and patience exhausted. Breaking training for current token." << std::endl;
                    break;
                }
                ...
                // this->otok is already a member
                // set zero for infinity and nan in otok (your existing logic)
                ...
                // use kernelComputePrediction for output prediction
                {
                    ...
                    try {
                        ...
                        CL_CHECK(kernel.setArg(4, static_cast<cl_int>(this->d)));
