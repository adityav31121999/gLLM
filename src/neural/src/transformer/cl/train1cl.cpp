...
void transformer::clTrain(std::vector<std::vector<float>>& sentence, std::vector<std::string>& rString)
{
    ...
    try {
        ...
        float learnBy = learning;   // Local copy of learning rate
        // --- Device Buffer Allocation & H->D Transfer ---
        size_t totalTokenEmbedFloats = static_cast<size_t>(m) * CONTEXT_WIN * d;
        size_t tokenEmbedBytes = totalTokenEmbedFloats * sizeof(float);
        size_t embeddingsBytes = static_cast<size_t>(vocabsize) * d * sizeof(float);
        ...
        for (size_t i = 1; i < sentence.size(); ++i) {
            ...
            int j = 1; // Epoch counter for this token
            current_error = 1.0f;
            prev_Error = 0.0f;

            std::cout << "Training token " << i+1 << "/" << sentence.size() << ": '" << expected_str << "'" << std::endl;
            ...
            while (j <= this->epochs) {
                ...
                // normalise the prediction raw logits
                ...
                std::cout << predicted_token_str << "\t | \t" << host_indexForToken << "\t | \t" 
                          << current_error << " , " << MAE(expected_vec, otok) << "\t | " 
                          << j << " | " << this->learning << std::endl;

                // --- Early exit if token is predicted correctly ---
                if (predicted_token_str == expected_str && predicted_token_str != "INVALID_INDEX") {
                    std::cout << "Token '" << expected_str << "' predicted correctly after " << j << " epochs. Moving to next token." << std::endl;
                    this->learning = learnBy;
                ...
                if(j == this->epochs - 1) {
                    std::cout << "Reached maximum epochs (" << this->epochs << ") for current token without correct prediction.\nIncreasing Epochs by 10." << std::endl;
                    this->epochs += 10;
                }
                ...
                if(current_block_idx == 1) {
                    // keys and queries for each head
                    ...
                    // keys and queries for all heads
                    for (int layer_idx = 0; layer_idx < x; ++layer_idx) {
                        for (int parallel_idx = 0; parallel_idx < y; ++parallel_idx) {
            ...
            learning = learnBy; // Reset learning rate for this epoch
        ...
        this->learning = learnBy; // Restore class learning rate
    }
