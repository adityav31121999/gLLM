...
/**
 * @brief (OpenCL) Train the transformer for prompt and response using embedding-deEmbedding.
 ...
 */
void transformer::clTrainContext(std::vector<std::vector<float>>& prompt, std::vector<std::vector<float>>& response, 
        std::vector<std::string>& rString)
{
    // --- Basic validation ---
    if (prompt.empty()) {
        throw std::runtime_error("clTrainCONTEXT(prompt-response): Initial prompt cannot be empty.");
    }
    ...
    if (response.empty() || response.size() != rString.size()) {
        throw std::runtime_error("clTrainCONTEXT(prompt-response): Response embeddings/strings mismatch or empty.");
    }
    if ((!prompt.empty() && prompt[0].size() != static_cast<size_t>(d)) || (!response.empty() && response[0].size() != static_cast<size_t>(d))) {
        throw std::runtime_error("clTrainCONTEXT(prompt-response): Embedding dimension mismatch.");
    }
    if (this->currentTokenCount + prompt.size() + response.size() > FULL_CONTEXT) {
        throw std::runtime_error("clTrainCONTEXT(prompt-response): Adding prompt and response exceeds FULL_CONTEXT limit.");
    }

    cl::Buffer d_tokenEmbed, d_deEmbeddings, d_expected_response_token;
    ...
    try {
        ...
        if (this->tokenEmbed.mapped_data && this->tokenEmbed.row >= static_cast<size_t>(this->currentTokenCount) && this->tokenEmbed.col == static_cast<size_t>(d)) {
            ...
            std::cerr << "Warning: Host tokenEmbed (mat) not properly initialized or too small for currentTokenCount ("
                      ...
                      << ") in clTrainCONTEXT(prompt-response) setup." << std::endl;
        ...
        std::vector<float> flat_deEmbeddings = ::flatten(this->deEmbeddings); // Assuming global or from basic.hpp
        ...
        if ((initial_token_count % CONTEXT_WIN) + prompt.size() > CONTEXT_WIN) {
            throw std::runtime_error("clTrainCONTEXT(prompt, response): Prompt exceeds current block capacity when starting.");
        }
        ...
        for (size_t p = 0; p < prompt.size(); ++p) {
            ...
            if (offset_bytes + singleTokenBytes > tokenEmbedBytes) {
                throw std::out_of_range("clTrainCONTEXT(prompt-response): Offset exceeds buffer bounds when writing prompt token.");
            }
            ...
            if (this->tokenEmbed.mapped_data && static_cast<size_t>(initial_token_count + p) < this->tokenEmbed.row && this->tokenEmbed.col == static_cast<size_t>(d)) {
                ...
                if (prompt[p].size() == static_cast<size_t>(d)) {
                    ...
                    std::cerr << "Error: prompt[p] size mismatch for host tokenEmbed (mat) update in clTrainCONTEXT(prompt-response)." << std::endl;
                }
            }
            else {
                std::cerr << "Error: Host tokenEmbed (mat) not properly initialized or out of bounds for prompt[p] update in clTrainCONTEXT(prompt-response)." << std::endl;
            }
        ...
        for (size_t i = 0; i < response.size(); ++i) {
            ...
            if (this->currentTokenCount >= FULL_CONTEXT) {
                std::cerr << "Warning: clTrainCONTEXT(prompt-response) reached FULL_CONTEXT limit ("
            ...
            if (current_block_idx <= 0 || current_block_idx > m) {
                throw std::out_of_range("clTrainCONTEXT(prompt-response): Calculated current_block_idx (" 
            ...
            while (j < this->epochs) {
                        ...
                        for (int layer_idx = 0; layer_idx < x; ++layer_idx) {
                            for (int parallel_idx = 0; parallel_idx < y; ++parallel_idx) {
                                ...
                                for (size_t k = 0; k < prompt.size(); ++k) {
                                    ...
                                    kq_kernel.setArg(3, embedding_dim_cl);
                                    kq_kernel.setArg(4, mat_heights_cl);
                ...
                if (h_otok_buffer.size() != static_cast<size_t>(d)) {
                    throw std::runtime_error("clTrainCONTEXT(prompt-response): this->otok from clForward has incorrect size. Expected " + std::to_string(d) + ", got " + std::to_string(this->otok.size()));
                }
                ...
                if (predicted_token_str == expected_str && predicted_token_str != "INVALID_INDEX") {
                    ...
                    if (offset_bytes + outputBytes > tokenEmbedBytes) {
                        throw std::out_of_range("clTrainCONTEXT(prompt-response): Offset exceeds buffer bounds when writing converged response token.");
                    }
            ...
            if (this->tokenEmbed.mapped_data && static_cast<size_t>(effective_context_size) < this->tokenEmbed.row && this->tokenEmbed.col == static_cast<size_t>(d)) {
                ...
                if (expected_vec.size() == static_cast<size_t>(d)) { // Ensure expected_vec is correctly sized
                    ...
                    std::cerr << "Error: expected_vec size mismatch for host tokenEmbed (mat) update in clTrainCONTEXT(prompt-response)." << std::endl;
                }
            } 
            else {
                std::cerr << "Error: Host tokenEmbed (mat) not properly initialized or out of bounds for h_otok_buffer update in clTrainCONTEXT(prompt-response)." << std::endl;
            }
            this->currentTokenCount++;
        ...
        std::cerr << "Standard Exception in rclTrainCONTEXT(prompt-response): " << e.what() << std::endl;
        this->epochs = initial_epochs;
        throw;
    }
}
...
/**
 * @brief (OpenCL) Train transformers for continuous chatsusing embedding-deEmbedding.
 *          Contextualisation for embeddings and deEmbeddings.
 ...
 */
void transformer::clTrainContext(std::vector<std::vector<std::vector<float>>>& prompts, std::vector<std::vector<std::vector<float>>>& responses,
                        std::vector<std::vector<std::string>>& rString)
{
    // --- Validation ---
    if (prompts.size() != responses.size() || responses.size() != rString.size()) {
        throw std::runtime_error("clTrainCONTEXT(chat): Mismatch in number of prompts (" + std::to_string(prompts.size())
    ...
    for (size_t i = 0; i < prompts.size(); ++i) {
        if (prompts[i].empty()) {
            throw std::runtime_error("clTrainCONTEXT(chat): Chat training requires non-empty prompts at index " + std::to_string(i));
        }
        if (responses[i].empty() || responses[i].size() != rString[i].size()) {
            throw std::runtime_error("clTrainCONTEXT(chat): Response embeddings/strings mismatch or empty at index " + std::to_string(i));
        }
        // Add dimension checks for embeddings
        if (!prompts[i].empty() && prompts[i][0].size() != static_cast<size_t>(d)) {
            throw std::runtime_error("clTrainCONTEXT(chat): Prompt embedding dimension mismatch at index " + std::to_string(i));
        }
         if (!responses[i].empty() && responses[i][0].size() != static_cast<size_t>(d)) {
            throw std::runtime_error("clTrainCONTEXT(chat): Response embedding dimension mismatch at index " + std::to_string(i));
        }
        total_tokens_to_add += prompts[i].size() + responses[i].size();
    }

    if (this->currentTokenCount + total_tokens_to_add > FULL_CONTEXT) {
        throw std::runtime_error("rclTrainCONTEXT(chat): Total tokens required (" 
    ...
    for (size_t i = 0; i < prompts.size(); ++i) {
        try {
            clTrainContext(prompts[i], responses[i], rString[i]);
        } 
        catch (const std::exception& e) {
