                        ...
                        std::vector<std::vector<std::string>>& rString)
{
    ...
    // Similar to cuTest, assuming each call to clTest(prompt,response,...) manages its own local state.
    // The main transformer's currentTokenCount, blockCount are effectively reset by the inner clTest calls.

    bool originalInTraining = this->inTraining;
    this->inTraining = false; // Ensure inference mode

    for (size_t i = 0; i < prompts.size(); ++i) {
        std::cout << "--- Testing Chat Turn (OpenCL) " << i + 1 << "/" << prompts.size() << " ---" << std::endl;
        clTest(prompts[i], responses[i], rString[i]); // Call single test
    }
