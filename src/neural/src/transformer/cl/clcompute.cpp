        ...
        for (int i = 0; i < num_heads_in_parallel; ++i) {
            ...
            if (inTraining) {
                ...
                if (effective_sequence1_len > 0) {
                    ...
                    if (blockCount == 1) {
                        if (isSelf) {
                            kernel = clcontext.kernels.at("kernelKdotQBlock1Self_Inference");
                            CL_CHECK(kernel.setArg(0, d_kdotq_head)); // Pass sub-buffer
                            ...
                            kernel = clcontext.kernels.at("kernelKdotQBlock1Cross_Inference");
                            CL_CHECK(kernel.setArg(0, d_kdotq_head)); // Pass sub-buffer
                        ...
                        if (isSelf) {
                            kernel = clcontext.kernels.at("kernelKdotQBlockNSelf_Inference");
                            CL_CHECK(kernel.setArg(0, d_kdotq_head)); // Pass sub-buffer
                            ...
                            kernel = clcontext.kernels.at("kernelKdotQBlockNCross_Inference");
                            CL_CHECK(kernel.setArg(0, d_kdotq_head)); // Pass sub-buffer
