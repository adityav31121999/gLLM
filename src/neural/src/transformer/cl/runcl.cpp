 ...
 */
void transformer::clRun(bool context)
{
    // Set for inference
    inTraining = 0;
    if (blocks.empty()) {
    ...
    try {
        ...
        if(context == 0) {
            ...
            // use the same buffer for de-embeddings
            d_embeddings = cl::Buffer(clcontext.context, CL_MEM_READ_ONLY, vocab_bytes * x, nullptr, &cl_err); CL_CHECK(cl_err); // Read-only for inference
        }
        ...
        if (context == 1) {
            ...
            if (deEmbeddings.row != vocabsize || deEmbeddings.col != d * x) {
                throw std::runtime_error("Vocabulary embeddings (mat) dimensions mismatch vocabsize or embedding dimension.");
            }
            if (deEmbeddings.mapped_size < vocab_bytes * x) {
                throw std::runtime_error("Vocabulary embeddings (mat) mapped size is insufficient.");
            }
        }

        CL_CHECK(clcontext.queue.enqueueWriteBuffer(d_embeddings, CL_TRUE, 0, vocab_bytes, (context == 0) ? embeddings.mapped_data : deEmbeddings.mapped_data));
            ...
            { // Scope for temporary prediction buffers
                ...
                try {
                    if (context == 0) {
                        ...
                        cl::Kernel predKernel = clcontext.kernels.at("kernelComputePredictionWithScores");
