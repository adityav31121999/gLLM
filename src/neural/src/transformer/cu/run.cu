 ...
 */
void transformer::cuRun(std::vector<std::vector<float>> &prompt) {
    // Set for inference
    inTraining = 0; 
    if (t.empty()) {
    ...
    try {
        ...
        while (1) {
            ...
            if (promptCount <= space_in_current_block) {
                ...
                if (blockCount > 1) {
                    // Copy the last CONTEXT_WIN embeddings from d_tokenEmbed to d_tokForBlock.
                    // This prepares the block-local context buffer for subsequent calculations
                    // within this block, mirroring the C++ logic.
                    // currentTokenCount includes the prompt tokens just added.
