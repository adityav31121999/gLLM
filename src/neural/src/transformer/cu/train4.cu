 ...
 */
void transformer::cuTrainContext(std::vector<std::vector<float>>& prompt, std::vector<std::vector<float>>& response,
 ...
 */
void transformer::cuTrainContext(std::vector<std::vector<std::vector<float>>>& prompts, std::vector<std::vector<std::vector<float>>>& responses,
                        std::vector<std::vector<std::string>>& rString)
{
    ...
    for(int i = 0; i < prompts.size(); ++i) {
        if (!prompts[i].empty() && !responses[i].empty()) {
            cuTrainContext(prompts[i], responses[i], rString[i]);
        }
        else {
