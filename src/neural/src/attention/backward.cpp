...
/**
 * @brief Backward Propagation for the attention class using gradients from expected Horizontal and Vertical output.
 *      Use for Second to Last second Block and repetitions (applicabel when there is continuation from previous blocks
 *      and repetitions).
 * @param expectedH Vertical embedding vector (context transfer to next head)
 * @param expectedV Horizontal embedding vector (next token prediction)
 * @param in Input size (number of tokens)
 * @param layers Number of layers in the MLPs
 */
void attention::backward(std::vector<float>& expectedH, std::vector<std::vector<float>>& expectedV, int& in, int& layers) 
{
    // Step 1: Compute loss gradient w.r.t. EH (for token prediction) and EV (for context)
    std::vector<float> grad_EH(EMBEDDING, 0.0f);
    std::vector<float> grad_EV(EMBEDDING, 0.0f);
    ...
    for(int i = 0; i < EMBEDDING; i++) {
        for(int j = 0; j < CONTEXT_WIN; j++) {
            ev[i] += EV[i][j];
            grad_EV[i] = 2.0f * (EV[i][j] - expectedV[i][j]); // MSE gradient for EV
        }
        ev[i] *= LEARNING;
        grad_EV[i] *= LEARNING;
    }
...
void attention::backward(std::vector<std::vector<float>>& expectedV, int& in, int& layers) 
{
    ...
    // Step 4: Compute gradients w.r.t. MH and MV
    // std::vector<std::vector<float>> grad_MH(MATHEIGHTS, std::vector<float>(EMBEDDING, 0.0f)); // Not needed
    std::vector<std::vector<float>> grad_MV(MATHEIGHTS, std::vector<float>(EMBEDDING, 0.0f));
    std::vector<float> pre_MH(MATHEIGHTS, 0.0f);
    std::vector<float> pre_MV(MATHEIGHTS, 0.0f);
    ...
    for (int h = 0; h < MATHEIGHTS; h++) {
        for (int d = 0; d < EMBEDDING; d++) {
            // grad_MH[h][d] = pre_MH[h] * grad_dh[d]; // Not needed
            grad_MV[h][d] = pre_MV[h] * grad_dv[d];
        }
    }
    ...
    for (int i = 0; i < MATHEIGHTS; i++) {
        for (int j = 0; j < EMBEDDING; j++) {
            // MH.a[j][i] -= LEARNING * grad_MH[j][i]; // Not needed
            MV.a[j][i] -= LEARNING * grad_MV[j][i];
            MQ.a[i][j] -= LEARNING * grad_MQ[i][j];
 ...
 * @param layers Number of layers in the MLPs
 */
void attention::backward1stHead(std::vector<float>& expected, int& in, int& layers) 
{
    // Step 1: Compute loss gradient w.r.t. EH (for token prediction) and EV (for context)
    std::vector<float> grad_EH(EMBEDDING, 0.0f);
    std::vector<float> grad_EV(EMBEDDING, 0.0f);
...
/**
 * @brief Backward Propagation (for first head) for the attention class using gradients from expected Horizontal and Vertical
 *      output. Use for Second to Last second Block and repetitions (applicabel when there is continuation from previous blocks
 *      and repetitions).
 * @param expectedH Vertical embedding vector (context transfer to next head)
 * @param expectedV Horizontal embedding vector (next token prediction)
 * @param in Input size (number of tokens)
 * @param layers Number of layers in the MLPs
 */
void attention::backward1stHead(std::vector<float>& expectedH, std::vector<std::vector<float>>& expectedV, int& in, int& layers) 
{
    // Step 1: Compute loss gradient w.r.t. EH (for token prediction) and EV (for context)
    std::vector<float> grad_EH(EMBEDDING, 0.0f);
    std::vector<float> grad_EV(EMBEDDING, 0.0f);
