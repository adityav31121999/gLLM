...
/**
 * @brief forward propagation for first block's attention class
...
/**
 * @brief forward propagation for a 2nd to last block's attention class
...
/**
 * @brief forward propagation for first block's attention class for continued forprop and shift to next block
 * @param in embedding dimension
 * @param layers layers of hidden weights in mlp
 * @param tokenCount token count for each attention head
 */
void attention::forpropev(int& in, int& layers, int& tokenCount)
{
    if (tokenCount <= 0 || tokenCount > KdotQ.row || tokenCount > KdotQ.col) {
        return;
    }
    ...
    for(int i = 0; i < tokenCount; ++i) {
        l_sum = 0.0f;   // column sum
        int limit_j = isSelfAttention ? (i + 1) : tokenCount;
        limit_j = std::min<float>(limit_j, head.col);
        for(int j = 0; j < limit_j; ++j) {
            if (j < head.row && i < head.col) {
                l_sum += head(j, i);
            }
...
/**
 * @brief forward propagation for a 2nd to last block's attention class for continued forprop and shift to next block
 * @param EVp EV matrix from previous block (passed by const reference)
 * @param in input token count
 * @param layers layers of MLPs
 * @param tokenCount number of tokens in full context
 ...
 * @param n number of tokens for each attention head (context window)
 */
void attention::forpropev(const mat& EVp, int& in, int& layers, int& tokenCount, int& blockCount, int& n) // Changed EVp type to const mat&
{
    if(blockCount == 0) {
        int firstBlockTokenCount = std::min(tokenCount, n);
        forprop(in, layers, firstBlockTokenCount); // Call the first block version
