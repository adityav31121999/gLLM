...
void attention::clbackward1stHead(std::vector<float>& expected, int& in, int& layers, int headnumber, float& learning)
{
    ...
    cl::Buffer d_EV(context, CL_MEM_READ_ONLY, ev_bytes, nullptr, &cl_err); CL_CHECK(cl_err); // Added d_EV for MLP input
    cl::Buffer d_grad_EH(context, CL_MEM_READ_WRITE, embed_bytes, nullptr, &cl_err); CL_CHECK(cl_err);
    cl::Buffer d_grad_EV_scaled(context, CL_MEM_READ_WRITE, embed_bytes, nullptr, &cl_err); CL_CHECK(cl_err); // Output for scaled EV gradient (grad_EH * 0.1f), used by 'ver' MLP
    ...
    cl::Buffer d_grad_MK(context, CL_MEM_READ_WRITE, mh_mv_mq_mk_bytes, nullptr, &cl_err); CL_CHECK(cl_err);
    ...
    // If this->ver.activations has 'layers' elements and d_ver_activations[layers-1] is used, copy all.
    for (int l = 0; l < layers; ++l) {
        CL_CHECK(queue.enqueueWriteBuffer(d_ver_activations[l], CL_TRUE, 0, embed_bytes, this->ver.activations[l].data()));
    }
    // this->hor.weights and d_hor_weights should have 'layers-1' elements
    for (int l = 0; l < layers - 1; ++l) {
    ...
    try {
        ...
        // K, Q are token_count x embedding_dim. grad_K, grad_Q are token_count x embedding_dim.
        cl::NDRange global_kq_grad_2d = calculate_global_2d(embedding_dim, token_count); // dim0 for embedding_dim (h), dim1 for token_count (i)
        ...
        CL_CHECK(grad_eh_ev_kernel.setArg(3, d_grad_EV_scaled)); // Write dummy value
        ...
        CL_CHECK(queue.finish()); // Sync before MLP backprop
        ...
        CL_CHECK(last_delta_kernel.setArg(0, d_grad_EV_scaled)); // Use scaled EV gradient
        ...
        CL_CHECK(lota_kernel.setArg(2, token_count)); // rows
        CL_CHECK(lota_kernel.setArg(3, token_count)); // cols
        ...
        cl::Kernel pre_mh_mv_kernel = context_obj.kernels.at("kernelComputePreMH_MV"); // Assumed name
        ...
        cl::Kernel grad_mh_mv_kernel = context_obj.kernels.at("kernelComputeGradMH_MV"); // Assumed name
        ...
        cl::Kernel grad_head_kernel = context_obj.kernels.at("kernelComputeGradHead"); // Assumed name
        ...
        CL_CHECK(lota_deriv_kernel.setArg(0, d_KdotQ)); // Input is KdotQ
        CL_CHECK(lota_deriv_kernel.setArg(1, d_lota_deriv));
        CL_CHECK(lota_deriv_kernel.setArg(2, token_count)); // rows
        CL_CHECK(lota_deriv_kernel.setArg(3, token_count)); // cols
        // Adjust launch for single work-group if needed
        if (this->KdotQ.row * this->KdotQ.col > 0) {
        ...
        cl::Kernel grad_k_q_kernel = context_obj.kernels.at("kernelComputeGradK_Q"); // Assumed name
        ...
        CL_CHECK(grad_k_q_kernel.setArg(6, embedding_dim)); // K, Q col dim is embedding_dim
        CL_CHECK(queue.enqueueNDRangeKernel(grad_k_q_kernel, cl::NullRange, global_kq_grad_2d, local_2d));
        ...
        cl::Kernel update_weights_h_kernel = context_obj.kernels.at("kernelUpdateWeights_1stHead_H"); // Assumed name
        cl_int cl_first = first; // Convert bool to cl_int
        CL_CHECK(update_weights_h_kernel.setArg(0, d_MH_a));
        CL_CHECK(update_weights_h_kernel.setArg(1, d_MV_a));
        CL_CHECK(update_weights_h_kernel.setArg(2, d_MQ_a));
        CL_CHECK(update_weights_h_kernel.setArg(3, d_MK_a));
        CL_CHECK(update_weights_h_kernel.setArg(4, d_EH));
        CL_CHECK(update_weights_h_kernel.setArg(5, d_grad_MH));
        CL_CHECK(update_weights_h_kernel.setArg(6, d_grad_MV));
        CL_CHECK(update_weights_h_kernel.setArg(7, d_grad_MQ));
        CL_CHECK(update_weights_h_kernel.setArg(8, d_grad_MK));
        CL_CHECK(update_weights_h_kernel.setArg(9, d_grad_EH));
        CL_CHECK(update_weights_h_kernel.setArg(10, learning_rate));
        CL_CHECK(update_weights_h_kernel.setArg(11, cl_first)); // Pass cl_int flag
        CL_CHECK(update_weights_h_kernel.setArg(12, mat_heights));
        CL_CHECK(update_weights_h_kernel.setArg(13, embedding_dim));
        CL_CHECK(queue.enqueueNDRangeKernel(update_weights_h_kernel, cl::NullRange, global_matrix, local_1d));
        CL_CHECK(queue.finish()); // Sync before D->H copy

        // --- Data Transfer D->H ---
        // Copy updated MLP weights and gradients back
        // MLP weight/gweight vectors have 'layers-1' elements
        for (int l = 0; l < layers - 1; ++l) {
            // Hor MLP
        ...
        // RAII will handle cl::Buffer cleanup
        throw;
    }
...
void attention::clbackward1stHead(std::vector<std::vector<float>>& expectedV, int& in, int& layers, float& learning)
{
    ...
    cl::Buffer d_EV(context, CL_MEM_READ_ONLY, ev_bytes, nullptr, &cl_err); CL_CHECK(cl_err); // Read-only
    ...
    std::vector<cl::Buffer> d_ver_gweights;
    std::vector<cl::Buffer> d_ver_deltas;
    d_ver_activations.reserve(layers);     // To store all activations including input layer
    d_ver_weights.reserve(layers-1);       // layers-1 weight matrices
    d_ver_gweights.reserve(layers-1);      // layers-1 gradient weight matrices
    d_ver_deltas.reserve(layers);          // To store all deltas
    ...
    for (int l = 0; l < layers; ++l) { // All 'layers' activations
        CL_CHECK(queue.enqueueWriteBuffer(d_ver_activations[l], CL_TRUE, 0, embed_bytes, this->ver.activations[l].data()));
    }
    for (int l = 0; l < layers-1; ++l) { // 'layers-1' weights
        CL_CHECK(queue.enqueueWriteBuffer(d_ver_weights[l], CL_TRUE, 0, mlp_weights_bytes, this->ver.weights[l].mapped_data));
    }
    ...
    try {
        ...
        for (int l = 0; l < layers - 1; ++l) { // Corrected loop bound
            // Assuming this->ver.activations[0] is the input to the ver MLP. d_EV is read-only and not the direct MLP input.
            cl::Buffer& d_prev_activations_ver = (l == 0) ? d_ver_activations[0] : d_ver_activations[l-1];
        ...
        CL_CHECK(grad_mlp_input_kernel.setArg(2, d_grad_dv));
        CL_CHECK(grad_mlp_input_kernel.setArg(3, embedding_dim));
        CL_CHECK(grad_mlp_input_kernel.setArg(4, embedding_dim)); // Assuming ver input size is embed_dim
        CL_CHECK(queue.enqueueNDRangeKernel(grad_mlp_input_kernel, cl::NullRange, global_embed, local_1d));
        CL_CHECK(queue.finish());
        ...
        CL_CHECK(grad_q_v_kernel.setArg(4, mat_heights));
        CL_CHECK(queue.enqueueNDRangeKernel(grad_q_v_kernel, cl::NullRange, global_kq_grad_2d, local_2d));
        ...
        CL_CHECK(grad_mq_v_kernel.setArg(1, d_null)); // d_Q_embed (nullptr)
        ...
        cl::Kernel update_weights_v_kernel = context_obj.kernels.at("kernelUpdateWeights_1stHead_V");
        CL_CHECK(update_weights_v_kernel.setArg(0, d_MV_a));
        CL_CHECK(update_weights_v_kernel.setArg(1, d_MQ_a));
        CL_CHECK(update_weights_v_kernel.setArg(2, d_MK_a));
        CL_CHECK(update_weights_v_kernel.setArg(3, d_grad_MV));
        CL_CHECK(update_weights_v_kernel.setArg(4, d_grad_MQ));
        CL_CHECK(update_weights_v_kernel.setArg(5, d_grad_MK_correction));
        CL_CHECK(update_weights_v_kernel.setArg(6, learning_rate));
        CL_CHECK(update_weights_v_kernel.setArg(7, mat_heights));
        CL_CHECK(update_weights_v_kernel.setArg(8, embedding_dim));
        CL_CHECK(queue.enqueueNDRangeKernel(update_weights_v_kernel, cl::NullRange, global_matrix, local_1d));
        CL_CHECK(queue.finish()); // Sync before D->H copy
    ...
    // Buffers are automatically released when they go out of scope (RAII)
    std::cout << "backward 1st head done" << std::endl;
}
