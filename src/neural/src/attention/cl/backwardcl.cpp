 ...
 *      Assumes necessary kernels are compiled and stored in clContext.kernels.
...
void attention::clbackward(std::vector<float>& expected, int& in, int& layers)
{
    ...
    cl::Buffer d_EH(context, CL_MEM_READ_WRITE, embed_bytes); // Read/Write for update
    cl::Buffer d_EV(context, CL_MEM_READ_WRITE, ev_size * sizeof(float)); // Read/Write for update
    ...
    try {
        ...
        grad_eh_ev_kernel.setArg(3, d_grad_EV_scaled);
        ...
        for (int l = 0; l < layers; ++l) {
            ...
            cl::Buffer& d_prev_activations = (l == 0) ? d_EV : d_ver_activations[l - 1];
            ...
            update_weights_kernel.setArg(5, embedding_dim);
            update_weights_kernel.setArg(6, prev_layer_size); // Input size might vary if not square
            queue.enqueueNDRangeKernel(update_weights_kernel, cl::NullRange, global_embed_2d, local_2d);
            queue.finish();
        }
        // --- End of MLP Backprop ---
        ...
        grad_mlp_input_kernel.setArg(3, embedding_dim); // current_layer_size
        grad_mlp_input_kernel.setArg(4, embedding_dim); // input_size
        ...
        cl::Buffer d_null; // Create a null cl::Buffer
        ...
        cl::Kernel update_weights_eh_ev_kernel = context_obj.kernels.at("clUpdateWeights_EH_EV"); // Assumed name
        update_weights_eh_ev_kernel.setArg(0, d_MH_a);
        update_weights_eh_ev_kernel.setArg(1, d_MV_a);
        update_weights_eh_ev_kernel.setArg(2, d_MQ_a);
        update_weights_eh_ev_kernel.setArg(3, d_MK_a);
        update_weights_eh_ev_kernel.setArg(4, d_EH);
        update_weights_eh_ev_kernel.setArg(5, d_EV);
        update_weights_eh_ev_kernel.setArg(6, d_grad_MH);
        update_weights_eh_ev_kernel.setArg(7, d_grad_MV);
        update_weights_eh_ev_kernel.setArg(8, d_grad_MQ);
        update_weights_eh_ev_kernel.setArg(9, d_grad_MK);
        update_weights_eh_ev_kernel.setArg(10, d_grad_EH); // Use original grad_EH
        update_weights_eh_ev_kernel.setArg(11, d_grad_EV_scaled); // Use scaled grad for EV update
        update_weights_eh_ev_kernel.setArg(12, learning_rate);
        update_weights_eh_ev_kernel.setArg(13, mat_heights);
        update_weights_eh_ev_kernel.setArg(14, embedding_dim);
        update_weights_eh_ev_kernel.setArg(15, context_win);
        // Launch with a grid size covering the largest update target (likely EV)
        queue.enqueueNDRangeKernel(update_weights_eh_ev_kernel, cl::NullRange, global_ev, local_1d);
        queue.finish();
        ...
        queue.enqueueReadBuffer(d_EV, CL_TRUE, 0, ev_size * sizeof(float), updated_EV.data()); // Read full EV
        ...
        std::cerr << "OpenCL Error in clbackward(expected): " << err.what() << " (" << err.err() << ")" << std::endl;
...
void attention::clbackward(std::vector<std::vector<float>>& expectedV, int& in, int& layers)
{
    ...
    try {
        ...
        grad_mq_v_kernel.setArg(1, d_null); // d_Q_embed
        ...
        cl::Kernel update_weights_ev_v_kernel = context_obj.kernels.at("clUpdateWeights_EV_V"); // Assumed name
        update_weights_ev_v_kernel.setArg(0, d_MV_a);
        update_weights_ev_v_kernel.setArg(1, d_MQ_a);
        update_weights_ev_v_kernel.setArg(2, d_MK_a);
        update_weights_ev_v_kernel.setArg(3, d_EV);
        update_weights_ev_v_kernel.setArg(4, d_grad_MV);
        update_weights_ev_v_kernel.setArg(5, d_grad_MQ);
        update_weights_ev_v_kernel.setArg(6, d_grad_MK_correction);
        update_weights_ev_v_kernel.setArg(7, d_grad_EV_full); // Use full EV gradient
        update_weights_ev_v_kernel.setArg(8, learning_rate);
        update_weights_ev_v_kernel.setArg(9, mat_heights);
        update_weights_ev_v_kernel.setArg(10, embedding_dim);
        update_weights_ev_v_kernel.setArg(11, context_win);
        // Launch with grid size covering largest update target (EV)
        queue.enqueueNDRangeKernel(update_weights_ev_v_kernel, cl::NullRange, global_ev, local_1d);
        queue.finish();
        ...
        std::cerr << "OpenCL Error in clbackward(expectedV): " << err.what() << " (" << err.err() << ")" << std::endl;
        // Manual Cleanup (Attention)
