...
/**
 * @brief attention head inference for first local context
 * @param tokens token embeddings
 * @param in dimension of token embedding
 * @param layers number of layers of mlp
 * @param tokenCount number of tokens full context
 */
void attention::inferHead(const mat& tokens, int &in, int &layers, int &tokenCount)
{
    ...
    for(int i = 0; i < tokenCount; ++i) {
        ...
        if (i < K.row && i < Q.row) {
            dh = dh + (k_sum * getRow(tokens, i)); // Vector addition and scalar multiplication
            dv = dv + (l_sum * getRow(tokens, i)); // Vector addition and scalar multiplication
        }
    }
    ...
    std::vector<float> reluv = ReLU(ver.output);
    for(int i = 0; i < CONTEXT_WIN; ++i) {
        if (i < EV.row) { // Ensure EV is large enough
            EV(i) += reluv; // Update the i-th row of *this* block's EV
        }
    }
}

/**
 * @brief attention head inference for subsequent blocks
 * @param EVp vertical retention vectors from previous block with same indices
 * @param tokForBlock token embeddings for current block
 * @param in dimensions of token embedding
 * @param layers number of layers of mlp
 * @param tokenCount number of tokens in full context
 * @param blockCount position of block in full context
 * @param n local context
 */
void attention::inferHead(const mat& EVp, const mat& tokForBlock, int &in, int &layers, int &tokenCount, int &blockCount, int &n)
{
    if(blockCount == 0) {
    ...
    int startTokenIndexInFullContext = (blockCount -1) * n;
    int endTokenIndexInFullContext = std::min(tokenCount, (blockCount) * n);
    ...
    for(int i = 0; i < currentBlockTokenCount; ++i) {
        ...
        if (i < K.row && i < Q.row) { // K.row and Q.row should match currentBlockTokenCount
            dh = dh + (k_sum * getRow(EVp, i)); // Vector addition and scalar multiplication
            dv = dv + (l_sum * getRow(tokForBlock, i)); // Vector addition and scalar multiplication
        }
    }
