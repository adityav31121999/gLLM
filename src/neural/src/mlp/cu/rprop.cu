...
void mlp::cuRprop(std::vector<std::vector<float>>& dataset, int layers_param, int in_param, float learning, int epochs_param) {
    ...
    // These will have the same dimensions as this->weights
    std::vector<mat> prev_gradients_mats;
    std::vector<mat> delta_weights_mats;

    prev_gradients_mats.resize(this->num_layers - 1);
    delta_weights_mats.resize(this->num_layers - 1);

    for (unsigned int l = 0; l < this->num_layers - 1; ++l) {
        prev_gradients_mats[l] = mat(this->weights[l].row, this->weights[l].col); // Create temp mapped file
        std::fill_n(prev_gradients_mats[l].mapped_data,
                    static_cast<size_t>(prev_gradients_mats[l].row) * prev_gradients_mats[l].col,
                    0.0f);

        delta_weights_mats[l] = mat(this->weights[l].row, this->weights[l].col); // Create temp mapped file
    ...
    for (unsigned int epoch = 0; epoch < epochs_param; ++epoch) {
        ...
        for (const auto& data : dataset) {
            ...
            // this->cuForward(this->layer_sizes[0], this->num_layers);
            // this->cuBackprop(this->num_layers, this->layer_sizes[0], learning); // Assuming cuBackprop updates this->gweights

            // Compute mean square error
            float error = 0.0f;
            for (unsigned int i = 0; i < this->output.size(); ++i) { // Use actual output size
                error += std::pow(expected[i] - output[i], 2);
            }
            if (!this->output.empty()) {
                error /= this->output.size();
            }
            totalError += error;
            ...
            try {
                ...
                for (unsigned int l = 0; l < (this->num_layers - 1); ++l) {
                    mat& current_weights_mat = this->weights[l];
                    mat& current_gradients_mat = this->gweights[l]; // Gradients computed by cuBackprop
