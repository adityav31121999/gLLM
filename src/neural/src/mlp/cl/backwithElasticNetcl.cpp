...
/**
 * @brief OpenCL implementation of backpropagation with Elastic Net regularization using shared OpenCLContext.
 *        Calculates deltas and updates weights using the Elastic Net update rule directly.
 ...
 */
void mlp::clBackwithElasticNet(int in, int layers, float learning) {
    float lambda_l1 = 0.01f; // L1 regularization parameter (hardcoded or pass as argument)
    float lambda_l2 = 0.01f; // L2 regularization parameter (hardcoded or pass as argument)

    // --- Basic Sanity Checks ---
    if (output.size() != static_cast<size_t>(in) || expected.size() != static_cast<size_t>(in)) {
        throw std::invalid_argument("MLP clBackwithElasticNet: Output/Expected vector size mismatch.");
    }
    if (weights.empty() || weights.size() != static_cast<size_t>(layers + 1)) {
         throw std::runtime_error("MLP clBackwithElasticNet: Weights vector size mismatch.");
    }
     if (layers > 0 && (activations.empty() || activations.size() != static_cast<size_t>(layers))) {
        throw std::runtime_error("MLP clBackwithElasticNet: Activations vector size mismatch.");
    }
    if (input.size() != static_cast<size_t>(in)) {
        throw std::runtime_error("MLP clBackwithElasticNet: Input vector size mismatch.");
    }
    for (size_t l = 0; l <= static_cast<size_t>(layers); ++l) {
        ...
        if (this->weights[l].row != in || this->weights[l].col != in) {
             ...
             std::cerr << "Warning: MLP clBackwithElasticNet: Weight mat dimensions at layer " << l << " ("
     ...
     if (layers > 0) {
        for (size_t l = 0; l < static_cast<size_t>(layers); ++l) {
            if (activations[l].empty() || activations[l].size() != static_cast<size_t>(in)) {
                throw std::runtime_error("MLP clBackwithElasticNet: Activation dimensions error at layer " + std::to_string(l));
            }
        }
     }
    ...
    try {
        ...
        cl::Kernel kernelUpdateWELasticNet = context_obj.kernels.at("kernelUpdateElasticNet"); // Elastic Net update kernel
        ...
        // 3. Update Weights using Elastic Net Kernel (Iterate through layers 0 to layers)
        for (int l = 0; l <= layers; ++l) {
            cl::Buffer& d_prev_activations_buffer = (l == 0) ? d_input : d_activations[l - 1];

            CL_CHECK(kernelUpdateWELasticNet.setArg(0, d_layer_deltas[l]));          // Deltas delta[l]
            CL_CHECK(kernelUpdateWELasticNet.setArg(1, d_prev_activations_buffer)); // Activations A[l-1] or input
            CL_CHECK(kernelUpdateWELasticNet.setArg(2, d_weights[l]));              // Weights W[l] (read/write)
            CL_CHECK(kernelUpdateWELasticNet.setArg(3, cl_learning));               // Learning rate
            CL_CHECK(kernelUpdateWELasticNet.setArg(4, cl_lambda_l1));              // L1 lambda parameter
            CL_CHECK(kernelUpdateWELasticNet.setArg(5, cl_lambda_l2));              // L2 lambda parameter
            CL_CHECK(kernelUpdateWELasticNet.setArg(6, cl_in));                     // current_layer_size
            CL_CHECK(kernelUpdateWELasticNet.setArg(7, cl_in));                     // prev_layer_size
            CL_CHECK(context_obj.queue.enqueueNDRangeKernel(kernelUpdateWELasticNet, cl::NullRange, global_2d, local_2d));
        ...
        std::cerr << "Error: Kernel not found in the shared OpenCLContext kernel map during clBackwithElasticNet. "
                  << "Ensure kernels 'kernelOutputDeltaSigmoid', 'kernelHiddenDeltaSigmoid', and 'kernelUpdateWeightsElasticNet' were provided during OpenCLContext initialization. "
                  << "Details: " << oor.what() << std::endl;
        throw; // Re-throw exception
    }
    catch (const std::exception& e) {
        std::cerr << "Standard Exception in mlp::clBackwithElasticNet: " << e.what() << std::endl;
