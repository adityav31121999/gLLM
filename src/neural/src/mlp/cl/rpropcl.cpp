...
/**
 * @brief OpenCL implementation of Rprop algorithm for MLP.
 * @param dataset Input dataset (vector of input samples)
...
void mlp::clRprop(std::vector<std::vector<float>>& dataset, int layers, int in, float learning, int epochs) {
    ...
    try {
        ...
        cl::Kernel kernelRprop(program, "kernelRpropUpdate");

        // --- Device Buffer Allocation (Allocate ONCE outside loops) ---
        ...
        for (int l = 0; l <= layers; ++l) {
            ...
            d_gradients[l] = cl::Buffer(context, CL_MEM_READ_ONLY, weights_size_bytes); // Gradients are input to Rprop kernel
            d_prev_gradients[l] = cl::Buffer(context, CL_MEM_READ_WRITE, weights_size_bytes);
            d_delta_weights[l] = cl::Buffer(context, CL_MEM_READ_WRITE, weights_size_bytes);

            // --- Initial Data Transfer for Rprop state ---
            std::vector<float> flat_prev_grad_l = flatten(prev_gradients[l]);
            std::vector<float> flat_delta_w_l = flatten(delta_weights[l]);
            queue.enqueueWriteBuffer(d_prev_gradients[l], CL_TRUE, 0, weights_size_bytes, flat_prev_grad_l.data());
            queue.enqueueWriteBuffer(d_delta_weights[l], CL_TRUE, 0, weights_size_bytes, flat_delta_w_l.data());
        }
        ...
        for (int epoch = 0; epoch < epochs; ++epoch) {
            ...
            for (size_t sample_idx = 0; sample_idx < dataset.size(); ++sample_idx) {
                ...
                // 2. Backward Pass (Calculate Gradients using clBackprop)
                // This updates host gweights and host weights (using standard GD step)
                ...
                for (int l = 0; l <= layers; ++l) {
                    ...
                    // and calculated gradients (this->gweights) H->D for this layer
                    ...
                    queue.enqueueWriteBuffer(d_gradients[l], CL_TRUE, 0, weights_size_bytes, flat_gradients_l.data());
                    // d_prev_gradients and d_delta_weights are already on device and hold state

                    // Launch Rprop Kernel for layer l
                    ...
                    queue.enqueueNDRangeKernel(kernelRprop, cl::NullRange, global_rprop, local_rprop);

                    // Read updated weights, prev_gradients, delta_weights D->H
                    queue.enqueueReadBuffer(d_weights[l], CL_TRUE, 0, weights_size_bytes, flat_weights_l.data());
                    std::vector<float> flat_prev_grad_l(in * in);
                    std::vector<float> flat_delta_w_l(in * in);
                    queue.enqueueReadBuffer(d_prev_gradients[l], CL_TRUE, 0, weights_size_bytes, flat_prev_grad_l.data());
                    queue.enqueueReadBuffer(d_delta_weights[l], CL_TRUE, 0, weights_size_bytes, flat_delta_w_l.data());
        ...
        throw std::runtime_error("OpenCL error during Rprop training.");
    }
