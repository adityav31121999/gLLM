...
/**
 * @brief OpenCL implementation of the simple backward propagation function.
...
void mlp::clBackward(int in, int layers, float learning) {
    ...
    try {
        ...
        cl::Kernel kernelUpdateW(program, "kernelUpdateWeights");
        ...
        cl::Buffer d_output_activations(context, CL_MEM_READ_ONLY, layer_size_bytes); // Assuming output holds *activated* values for kernel
        ...
        cl::Buffer d_weights_next(context, CL_MEM_READ_ONLY, weights_size_bytes); // W[l+1]

        // Ping-pong buffers for deltas
        cl::Buffer d_delta_buffer1(context, CL_MEM_READ_WRITE, layer_size_bytes);
        cl::Buffer d_delta_buffer2(context, CL_MEM_READ_WRITE, layer_size_bytes);
        ...
        cl::Buffer d_weights_l(context, CL_MEM_READ_WRITE, weights_size_bytes); // W[l]
        cl::Buffer d_activations_prev(context, CL_MEM_READ_ONLY, layer_size_bytes); // Input or A[l-1]
        cl::Buffer d_input_buffer(context, CL_MEM_READ_ONLY, layer_size_bytes); // Separate buffer for initial input
        ...
        // If it holds pre-activation, this kernel call is technically incorrect vs CPU sigmoidder(output[i]).
        ...
        queue.enqueueWriteBuffer(d_input_buffer, CL_TRUE, 0, layer_size_bytes, input.data());
        for (int l = 0; l < layers; ++l) {
        ...
        for (int l = layers - 1; l >= 0; --l) {
            ...
            queue.enqueueWriteBuffer(d_weights_next, CL_TRUE, 0, weights_size_bytes, flat_weights_next.data());
        ...
        for (int l = 0; l <= layers; ++l) {
            ...
            queue.enqueueWriteBuffer(d_weights_l, CL_TRUE, 0, weights_size_bytes, flat_weights_l.data());
            ...
            queue.enqueueNDRangeKernel(kernelUpdateW, cl::NullRange, global_2d, local_2d); // Use 2D launch

            // Read updated weights back to host
            queue.enqueueReadBuffer(d_weights_l, CL_TRUE, 0, weights_size_bytes, flat_weights_l.data());
        ...
        throw std::runtime_error("OpenCL error during backward propagation.");
    } 
