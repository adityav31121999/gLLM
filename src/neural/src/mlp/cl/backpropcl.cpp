...
// Global OpenCL objects are declared in opencl_setup.hpp
...
/**
 * @brief OpenCL implementation of backpropagation for MLP with gradients.
...
void mlp::clBackprop(int in, int layers, float learning) {
    ...
    try {
        ...
        cl::Kernel kernelUpdateWAndG(program, "kernelUpdateWeightsAndGradients"); // Kernel that calculates gradients
        ...
        for (int l = 0; l <= layers; ++l) {
            d_weights[l] = cl::Buffer(context, CL_MEM_READ_WRITE, weights_size_bytes); // Weights are updated
            d_gweights[l] = cl::Buffer(context, CL_MEM_WRITE_ONLY, weights_size_bytes); // Gradients are calculated
            d_layer_deltas[l] = cl::Buffer(context, CL_MEM_READ_WRITE, layer_size_bytes); // Deltas are read/written
        }
        ...
        queue.enqueueWriteBuffer(d_output_activations, CL_TRUE, 0, layer_size_bytes, output.data()); // Assuming output holds activated values
        ...
        for (int l = layers - 1; l >= 0; --l) {
            ...
            kernelHiddenDelta.setArg(2, d_activations[l]);      // Activations of the current layer (l) (CORRECTED)
        ...
        for (int l = 0; l <= layers; ++l) {
            ...
            queue.enqueueNDRangeKernel(kernelUpdateWAndG, cl::NullRange, global_2d, local_2d);
        }

        // --- Data Transfer: Device -> Host ---
        // Wait for kernels to finish before reading back (or use blocking reads)
        queue.finish();
        ...
        for (int l = 0; l <= layers; ++l) {
            ...
            queue.enqueueReadBuffer(d_gweights[l], CL_TRUE, 0, weights_size_bytes, calculated_flat_gradients.data());
        ...
        throw std::runtime_error("OpenCL error during backpropagation.");
    } 
...
/**
 * @brief OpenCL implementation of backpropagation that also updates the input vector.
...
void mlp::clBackprop2in(int in, int layers, float learning) {
    ...
    try {
        ...
        cl::Kernel kernelOutDelta(program, "kernelOutputDeltaSigmoid");
        cl::Kernel kernelHiddenDelta(program, "kernelHiddenDeltaSigmoid");
        cl::Kernel kernelUpdateWAndG(program, "kernelUpdateWeightsAndGradients");
        cl::Kernel kernelUpdateIn(program, "kernelUpdateInputMLP"); // Kernel to update input
        ...
        // Buffers for inputs/outputs/activations
        cl::Buffer d_input(context, CL_MEM_READ_WRITE, layer_size_bytes); // Input is updated
        cl::Buffer d_output_activations(context, CL_MEM_READ_ONLY, layer_size_bytes);
        cl::Buffer d_expected(context, CL_MEM_READ_ONLY, layer_size_bytes);
        ...
        for (int l = 0; l <= layers; ++l) {
            ...
            d_gweights[l] = cl::Buffer(context, CL_MEM_WRITE_ONLY, weights_size_bytes);
            d_layer_deltas[l] = cl::Buffer(context, CL_MEM_READ_WRITE, layer_size_bytes);
        }
        for (int l = 0; l < layers; ++l) {
             d_activations[l] = cl::Buffer(context, CL_MEM_READ_ONLY, layer_size_bytes);
        }
        ...
        // --- Data Transfer: Host -> Device ---
        queue.enqueueWriteBuffer(d_input, CL_TRUE, 0, layer_size_bytes, input.data()); // Initial input
        ...
        for (int l = layers - 1; l >= 0; --l) {
            ...
            kernelHiddenDelta.setArg(2, d_activations[l]); // CORRECTED
        ...
        kernelUpdateIn.setArg(4, cl_in);                // first_hidden_layer_size
        kernelUpdateIn.setArg(5, cl_in);                // input_size
        queue.enqueueNDRangeKernel(kernelUpdateIn, cl::NullRange, global_1d, local_1d); // 1D kernel over input size

        // --- Data Transfer: Device -> Host ---
        queue.finish(); // Ensure all kernels are done before reading
        ...
        queue.enqueueReadBuffer(d_input, CL_TRUE, 0, layer_size_bytes, input.data());
        ...
        throw std::runtime_error("OpenCL error during backpropagation to input.");
    } 
    catch (const std::exception& e) {
