...
void mlp::clForward(int in_param, int layers_param) {
    ...
    try {
        ...
        // Initial input
        size_t input_layer_size_bytes = layer_sizes[0] * sizeof(float);
        d_prev_layer_activations = cl::Buffer(context_obj.context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                              input_layer_size_bytes, activations[0].data(), &err);
        CL_CHECK(err);

        // Iterate through weight matrices (num_layers - 1 of them)
        for (unsigned int l_idx = 0; l_idx < num_layers - 1; ++l_idx) {
            const mat& current_weights_mat = weights[l_idx];
            unsigned int current_input_neuron_count = layer_sizes[l_idx];
            unsigned int current_output_neuron_count = layer_sizes[l_idx+1];
            ...
            if (hlayers[l_idx].size() != current_output_neuron_count) {
                hlayers[l_idx].resize(current_output_neuron_count);
            }
            if (activations[l_idx+1].size() != current_output_neuron_count) {
                activations[l_idx+1].resize(current_output_neuron_count);
            }
            CL_CHECK(context_obj.queue.enqueueReadBuffer(d_current_hlayer_output, CL_TRUE, 0, output_hlayer_bytes, hlayers[l_idx].data()));
            CL_CHECK(context_obj.queue.enqueueReadBuffer(d_current_activation_output, CL_TRUE, 0, output_activation_bytes, activations[l_idx+1].data()));
        }
