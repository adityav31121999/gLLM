...
void mlp::clBackwithL1(int in, int layers, float learning) {
    ...
    for (size_t l = 0; l <= static_cast<size_t>(layers); ++l) {
        if (weights[l].empty() || weights[l].size() != static_cast<size_t>(in) || weights[l][0].size() != static_cast<size_t>(in)) {
             throw std::runtime_error("MLP clBackwithL1: Weight dimensions error at layer " + std::to_string(l));
        }
    }
     if (layers > 0) {
        for (size_t l = 0; l < static_cast<size_t>(layers); ++l) {
    ...
    try {
        ...
        for (int l = 0; l <= layers; ++l) {
            ...
            kernelUpdateWL1.setArg(0, d_weights[l]);              // Weights W[l] (read/write)
            kernelUpdateWL1.setArg(1, d_layer_deltas[l]);         // Deltas delta[l]
            kernelUpdateWL1.setArg(2, d_prev_activations_buffer); // Activations A[l-1] or input
            kernelUpdateWL1.setArg(3, cl_learning);               // Learning rate
