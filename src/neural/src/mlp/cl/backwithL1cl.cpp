...
void mlp::clBackwithL1(int in, int layers, float learning) {
    ...
    try {
        ...
        for (int l = 0; l <= layers; ++l) {
            ...
            CL_CHECK(kernelUpdateWL1.setArg(1, d_layer_deltas[l]));         // Deltas delta[l]
            CL_CHECK(kernelUpdateWL1.setArg(2, d_prev_activations_buffer)); // Activations A[l-1] or input
            CL_CHECK(kernelUpdateWL1.setArg(3, cl_learning));               // Learning rate
            CL_CHECK(kernelUpdateWL1.setArg(4, cl_lambda));                 // L1 lambda parameter
            CL_CHECK(kernelUpdateWL1.setArg(5, cl_in));                     // current_layer_size
            CL_CHECK(kernelUpdateWL1.setArg(6, cl_in));                     // prev_layer_size
            CL_CHECK(context_obj.queue.enqueueNDRangeKernel(kernelUpdateWL1, cl::NullRange, global_2d, local_2d));
