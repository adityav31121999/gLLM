 ...
 * @param layers layers of MLPs
 */
void block::backward1stBlock(std::vector<std::vector<float>>& expectedH, int& in, int& layers, int blockCount)
{
 ...
 * @param layers layers of MLPs
 */
void block::backward1stBlock(std::vector<float>& expectedH, int& in, int& layers, int blockCount)
{
    // run partial backpropagation in parallel
    for(int i = x-1; i >= 0; i++) {
        // expectedV for each is not considered since the horizontal pass ended in this block
 ...
 * @param layers layers of MLPs
 */
void block::backward(std::vector<std::vector<std::vector<std::vector<float>>>>& expectedV, int& in, int& layers, int blockCount)
{
 ...
 * @param layers layers of MLPs
 */
void block::backward(std::vector<float>& expectedH, int& in, int& layers, int blockCount) {
    // run partial backpropagation in parallel
    for(int i = x-1; i >= 0; i++) {
        // expectedV for each is not considered since the horizontal pass ended in this block
