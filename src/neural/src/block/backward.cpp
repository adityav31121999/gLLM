...
void block::backward1stBlock(std::vector<float>& expectedH, int& in, int& layers)
{
    ...
    for(int i = y; i >= 1; i--) {
        if(i == y) {
            ...
            partialbackward1stBlock(h2, in, layers, i);
        }
        else if(i == 1) {
            std::vector<std::vector<float>> h2(x, std::vector<float>(in, 0.0f));
            for(int j = 0; j < x; j++) {
...
void block::backward1stBlock(std::vector<std::vector<float>>& expectedH, int& in, int& layers)
{
    ...
    for(int i = y; i >= 1; i--) {
        if(i == y) {
            // for last column
            partialbackward1stBlock(expectedH[i-1], in, layers, i);
        }
        else if(i < y and i > 1) {
            std::vector<std::vector<float>> h2(x, std::vector<float>(in, 0.0f));
            for(int j = 0; j < x; j++) {
...
void block::backward(std::vector<float>& expectedH, int& in, int& layers, int blockCount) {
    ...
    for(int i = y; i >= 1; i--) {
        if(i == y) {
            ...
            // for last second to second column
            partialbackward(h2, in, layers, i);
        }
        else if(i == 1) {
            std::vector<std::vector<float>> h2(x, std::vector<float>(in, 0.0f));
            for(int j = 0; j < x; j++) {
...
{
    for(int i = y; i >= 1; i--) {
        partialbackward(expectedV[i-1], layers, i, blockCount);
        /*if(i == y) {
