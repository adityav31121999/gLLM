...
void block::cupartialbackward1stBlock(std::vector<std::vector<float>>& expectedH, int& in, int& layers, int& layno, float& learning, float& lambda_l1, float& lambda_l2)
{
    ...
    try {
        ...
        for (int i = num_heads_to_process - 1; i >= 0; --i) { // 'i' is the row index, also used as head_idx
            ...
            // In clpartialbackward1stBlock (OpenCL), kernelUpdateWeightsHeadElastic is used.
            // This kernel updates MH, MV, MQ, MK, and EH. It does NOT update EV.
            // The update_eh_flag is set to 1 (true) in the OpenCL counterpart.
            int update_eh_flag = 1; // Always update EH for the first block's backprop

            kernelUpdateWeightsHeadElastic<<<gridDimProjMat, blockDim1D, 0, current_stream>>>(
...
void block::cupartialbackward(std::vector<float> &expectedH, int& in, int& layers_mlp, int& layno, float& learning, float& lambda_l1, float& lambda_l2)
{
    ...
    std::vector<HeadDevicePointers> head_gpu_data(num_heads_to_process);
        ...
        try {
            ...
            // Step 8: Compute grad_MK and grad_MQ (Simplified)
            kernelComputeGradMK_MQ_Simplified<<<gridDimMatrix2D, blockDim2D, 0, current_stream>>>(device_ptrs.d_grad_K, device_ptrs.d_grad_Q, nullptr, nullptr, device_ptrs.d_grad_MK, device_ptrs.d_grad_MQ, token_count, mat_heights, embedding_dim); CUDA_CHECK(cudaGetLastError());
            /*if (token_count > 0) {
            ...
            // In clpartialbackward (OpenCL), kernelUpdateWeightsHeadHVElastic is used.
            // update_eh_flag is conditional on layno_col_idx > 0.
            // update_ev_flag is set to 0 (false) in the OpenCL counterpart.
            int update_eh_flag = (layno > 0) ? 1 : 0;
            int update_ev_flag = 0; // EV is not updated in this path in the OpenCL counterpart
