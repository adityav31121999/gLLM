 ...
 */
void block::cuParallelKdotQ(int& columnNumber, int& blockNumber, int& tokenCount, int& promptCount, bool isSelfAttention)
{
    ...
    for (int i = 0; i < num_heads_in_column; ++i) {
        ...
        // --- Validate K/Q Data ---
        if (head.K.size() < num_keys_eff || head.Q.size() < num_queries_eff || (num_keys_eff > 0 && 
            (head.K[0].size() != embedding_dim || head.Q[0].size() != embedding_dim)))
        {
            fprintf(stderr, "Error: Training K/Q size mismatch for head (%d, %d). K=%zu Q=%zu Expected=%d Emb=%d\n",
                    i, columnNumber, head.K.size(), head.Q.size(), num_keys_eff, embedding_dim);
            continue; // Skip this head
        }

        // --- GPU Allocation & Copy ---
        float *d_kdotq = nullptr, *d_keys = nullptr, *d_querys = nullptr;
        size_t keys_size_bytes = (size_t)num_keys_eff * embedding_dim * sizeof(float);
        size_t querys_size_bytes = (size_t)num_queries_eff * embedding_dim * sizeof(float);
        ...
        CUDA_CHECK(cudaMallocAsync(&d_keys, keys_size_bytes, current_stream));
        CUDA_CHECK(cudaMallocAsync(&d_querys, querys_size_bytes, current_stream));

        std::vector<float> flat_K = flatten(head.K); // Flatten first num_keys_eff rows
        std::vector<float> flat_Q = flatten(head.Q); // Flatten first num_queries_eff rows
        ...
        CUDA_CHECK(cudaMemcpyAsync(d_querys, flat_Q.data(), querys_size_bytes, cudaMemcpyHostToDevice, current_stream));
        ...
        CUDA_CHECK(cudaMemcpyAsync(flat_kdotq_results[i].data(), d_kdotq, kdotq_size_bytes, cudaMemcpyDeviceToHost, current_stream));
                              ...
                              int& columnNumber, int& tokenCount, int& promptCount, bool isSelfAttention)
{
    ...
    for (int i = 0; i < num_heads_in_column; ++i) {
        ...
        CUDA_CHECK(cudaMallocAsync(&d_tokenEmbed, embed_size_bytes, current_stream)); // Use pre-calculated size
        CUDA_CHECK(cudaMallocAsync(&d_M, M_size_bytes, current_stream));

        CUDA_CHECK(cudaMemcpyAsync(d_tokenEmbed, flat_tokenEmbed.data(), embed_size_bytes, cudaMemcpyHostToDevice, current_stream));
        CUDA_CHECK(cudaMemcpyAsync(d_M, flat_M.data(), M_size_bytes, cudaMemcpyHostToDevice, current_stream));
                              ...
                              int& columnNumber, int& blockNumber, int& tokenCount, int& promptCount, bool isSelfAttention)
{
    ...
    if (this->tokForBlock.empty() || this->tokForBlock.size() < context_len_block || 
    (context_len_block > 0 && this->tokForBlock[0].size() != embedding_dim)) 
    {
        fprintf(stderr, "Error: Invalid tokForBlock for Block %d inference. Need %d rows, %d emb; Have %zu rows\n",
                blockNumber, context_len_block, embedding_dim, this->tokForBlock.size());
        for (int i = 0; i < num_heads_in_column; ++i) cudaStreamDestroy(streams[i]);
    ...
    for (int i = 0; i < num_heads_in_column; ++i) {
        ...
        CUDA_CHECK(cudaMallocAsync(&d_kdotq, kdotq_size_bytes, current_stream));
        CUDA_CHECK(cudaMemsetAsync(d_kdotq, 0, kdotq_size_bytes, current_stream));
        CUDA_CHECK(cudaMallocAsync(&d_tokForBlock, tok_size_bytes, current_stream)); // Use pre-calculated size
        CUDA_CHECK(cudaMallocAsync(&d_EVp, evp_size_bytes, current_stream));
        CUDA_CHECK(cudaMallocAsync(&d_M, M_size_bytes, current_stream));

        CUDA_CHECK(cudaMemcpyAsync(d_tokForBlock, flat_tokForBlock.data(), tok_size_bytes, cudaMemcpyHostToDevice, current_stream));
        CUDA_CHECK(cudaMemcpyAsync(d_EVp, flat_EVp.data(), evp_size_bytes, cudaMemcpyHostToDevice, current_stream));
        CUDA_CHECK(cudaMemcpyAsync(d_M, flat_M.data(), M_size_bytes, cudaMemcpyHostToDevice, current_stream));
