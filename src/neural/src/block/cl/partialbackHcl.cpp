 ...
 * @param expectedH expected token embedding
    ...
    float& learning, float& lambda_l1, float& lambda_l2, float& clip_norm)
{
    ...
    if (layno_col_idx < 0 || layno_col_idx >= y) {
        throw std::out_of_range("clpartialbackward1stBlock(H): Column index 'layno_col_idx' (" + std::to_string(layno_col_idx) +
            ") is out of range [0, " + std::to_string(y - 1) + "].");
    }
    if (expectedH.size() != EMBEDDING) {
        throw std::runtime_error("ExpectedH vector size mismatch in clpartialbackward1stBlock(H). Expected " +
    ...
    // MODIFICATION: Reduced local work sizes for better compatibility
    const size_t local_work_size_1d = 64; // Was 256
    cl::NDRange local_1d(local_work_size_1d);
    const size_t local_work_size_2d_arr[2] = { 8, 8 }; // Was {16, 16}
    ...
    try {
        ...
        for (int head_idx = num_heads_to_process - 1; head_idx >= 0; --head_idx) {
            ...
            for (int l = 0; l < num_weight_matrices_mlp; ++l) { 
                ...
                CL_CHECK(k_update_weights_h.setArg(2, device_ptrs.d_hor_weights[l])); // weights (used for L2 regularization component)
                CL_CHECK(k_update_weights_h.setArg(3, device_ptrs.d_hor_gweights[l])); // This buffer receives the computed gradient
                CL_CHECK(k_update_weights_h.setArg(4, learning_rate));
                CL_CHECK(k_update_weights_h.setArg(5, lambda_l1)); // lambda_l1
                CL_CHECK(k_update_weights_h.setArg(6, lambda_l2)); // lambda_l2
            ...
            for (int l = 0; l < num_weight_matrices_mlp; ++l) { 
                ...
                CL_CHECK(k_update_weights_v.setArg(2, device_ptrs.d_ver_weights[l])); // weights (used for L2 regularization component)
                CL_CHECK(k_update_weights_v.setArg(3, device_ptrs.d_ver_gweights[l])); // This buffer receives the computed gradient
...
/*
            // Weight Updates (now using potentially clipped gradients)
            ...
            CL_CHECK(k_update_weights_eh_ev.setArg(17, host_max_grad_clip_value));
            CL_CHECK(k_update_weights_eh_ev.setArg(18, mat_heights));
            CL_CHECK(k_update_weights_eh_ev.setArg(19, embedding_dim));
            CL_CHECK(k_update_weights_eh_ev.setArg(20, context_win));
...
*/
            ...
            CL_CHECK(k_update_1st_h_2.setArg(12, lambda_l2));
...
/**
 * @brief backward propagation for last column of ith block (backprop starts from this)
 * @param expectedH expected token embedding
 * @param in embedding dimension and input-output vector dimension of mlp
 * @param layers number of layers of activations of mlp
    ...
    float& learning, float& lambda_l1, float& lambda_l2, float& clip_norm)
{
    ...
    if (layno_col_idx < 0 || layno_col_idx >= y) {
        throw std::out_of_range("clpartialbackward(H): Column index 'layno_col_idx' (" + std::to_string(layno_col_idx) +
            ") is out of range [0, " + std::to_string(y - 1) + "].");
    }
    if (expectedH.size() != EMBEDDING) {
        throw std::runtime_error("ExpectedH vector size mismatch in clpartialbackward(H). Expected " +
            std::to_string(EMBEDDING) + ", got " + std::to_string(expectedH.size()));
    }
    if (EMBEDDING != in_dim) {
        throw std::runtime_error("Embedding dimension mismatch: EMBEDDING vs in_dim");
    ...
    try {
        ...
        for (int head_idx = num_heads_to_process - 1; head_idx >= 0; --head_idx) {
            ...
            if (token_count > 0) {
                ...
                cl::Kernel k_grad_mk_mq = clcontext.kernels.at("kernelComputeGradMK_MQ"); // Corrected access to kernel
                CL_CHECK(k_grad_mk_mq.setArg(0, device_ptrs.d_grad_K));
                CL_CHECK(k_grad_mk_mq.setArg(1, device_ptrs.d_grad_Q));
                CL_CHECK(k_grad_mk_mq.setArg(2, device_ptrs.d_K));
                CL_CHECK(k_grad_mk_mq.setArg(3, device_ptrs.d_Q));
