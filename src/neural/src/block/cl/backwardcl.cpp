 ...
 *        Matches signature: void block::clBackward1stBlock(cl_context context, cl_command_queue queue, std::map<std::string, cl_kernel>& kernels, std::vector<float>& expectedH, int& in, int& layers);
 *
 ...
 *        Matches signature: void block::clBackward1stBlock(cl_context context, cl_command_queue queue, std::map<std::string, cl_kernel>& kernels, std::vector<std::vector<float>>& expectedH, int& in, int& layers);
 *
 * @param context OpenCL context.
 * @param queue OpenCL command queue.
 * @param kernels Map of compiled OpenCL kernels.
 ...
 *        Matches signature: void block::clBackward1stBlock(cl_context context, cl_command_queue queue, std::map<std::string, cl_kernel>& kernels, std::vector<std::vector<std::vector<std::vector<float>>>>& expectedV, int& in, int& layers);
 *
 * @param context OpenCL context.
 * @param queue OpenCL command queue.
 * @param kernels Map of compiled OpenCL kernels.
 ...
 *        Applies the same expectedH to all columns. Iterates columns in reverse.
 *        Matches signature: void block::clBackward(cl_context context, cl_command_queue queue, std::map<std::string, cl_kernel>& kernels, std::vector<float>& expectedH, int& in, int& layers);
 *
 * @param context OpenCL context.
 * @param queue OpenCL command queue.
 * @param kernels Map of compiled OpenCL kernels.
 ...
 * @param layers Number of MLP layers.
 */
void block::clBackward(std::vector<float>& expectedH, int& in, int& layers)
{
    // Validate input size
    if (expectedH.size() != EMBEDDING) {
 ...
 * @param layers Number of MLP layers.
 */
void block::clBackward(std::vector<std::vector<float>>& expectedH, int& in, int& layers)
{
    // Validate input size - should have one vector per column
    if (expectedH.size() != static_cast<size_t>(this->y)) {
 ...
 * @param layers Number of MLP layers.
 */
void block::clBackward(std::vector<std::vector<std::vector<std::vector<float>>>>& expectedV, int& in, int& layers)
{
    // Validate input dimensions
    if (expectedV.size() != static_cast<size_t>(this->x)) {
