...
void block::clpartialbackward1stBlock(std::vector<std::vector<float>>& expectedH, int &in_dim, int &layers_mlp, int& layno_col_idx,
    float& learning, float& lambda_l1, float& lambda_l2, float& clip_norm)
{
    cl_int cl_err;
    ...
    float host_max_grad_clip_value = MAX_GRAD_CLIP;
    ...
    try {
        cl::Buffer d_dL_dTokenEmbed_aggregated_column_output(clcontext.context, CL_MEM_READ_WRITE, static_cast<size_t>(this->tokenCount) * embedding_dim * sizeof(float), nullptr, &cl_err); 
        CL_CHECK(cl_err);
        CL_CHECK(clcontext.queue.enqueueFillBuffer(d_dL_dTokenEmbed_aggregated_column_output, 0.0f, 0, static_cast<size_t>(this->tokenCount) * embedding_dim * sizeof(float)));
        ...
        if(layno_col_idx == 0) {
            gradForTokens.resize(tokenCount, std::vector<float>(EMBEDDING, 0.0f));
            agg_d_grad_tokens = cl::Buffer(clcontext.context, CL_MEM_READ_WRITE, num_heads_to_process * token_embed_bytes, nullptr, &cl_err); CL_CHECK(cl_err);
        }
        ...
        for (int head_idx = 0; head_idx < num_heads_to_process; ++head_idx) {
            ...
            if(layno_col_idx == 0){
                region = { head_idx * token_embed_bytes, token_embed_bytes };
        ...
        for (int head_idx = num_heads_to_process - 1; head_idx >= 0; --head_idx) {
            ...
            bool is_first_head_of_block = (layno_col_idx == 0);
            ...
            for (int l = 0; l < num_weight_matrices_mlp; ++l) { 
                ...
                CL_CHECK(k_update_weights_h.setArg(7, host_max_grad_clip_value));
                CL_CHECK(k_update_weights_h.setArg(8, embedding_dim)); // current_layer_size
                CL_CHECK(k_update_weights_h.setArg(9, embedding_dim)); // prev_layer_size
            ...
            for (int l = 0; l < num_weight_matrices_mlp; ++l) { 
                ...
                CL_CHECK(k_update_weights_v.setArg(7, host_max_grad_clip_value));
                CL_CHECK(k_update_weights_v.setArg(8, embedding_dim)); // current_layer_size
                CL_CHECK(k_update_weights_v.setArg(9, embedding_dim)); // prev_layer_size
            ...
            // --- Apply Gradient Clipping for d_grad_dh ---
            ...
            // Args 3 and 4 (embedding_dim, embedding_dim) are already set
            CL_CHECK(current_stream.enqueueNDRangeKernel(k_grad_mlp_input, cl::NullRange, global_embed, local_1d));
            // --- Apply Gradient Clipping for d_grad_dv ---
            ...
            cl_int cl_update_eh_flag_1sth_2 = (layno_col_idx > 0) ? 1 : 0;
            cl::Kernel k_update_1st_h_2 = clcontext.kernels.at("kernelUpdateWeights_1stHead_H_ElasticNet");
            CL_CHECK(k_update_1st_h_2.setArg(0, device_ptrs.d_MH_a));
            CL_CHECK(k_update_1st_h_2.setArg(1, device_ptrs.d_MV_a));
            CL_CHECK(k_update_1st_h_2.setArg(2, device_ptrs.d_MQ_a));
            CL_CHECK(k_update_1st_h_2.setArg(3, device_ptrs.d_MK_a));
            CL_CHECK(k_update_1st_h_2.setArg(4, device_ptrs.d_EH));
            CL_CHECK(k_update_1st_h_2.setArg(5, device_ptrs.d_grad_MH));
            CL_CHECK(k_update_1st_h_2.setArg(6, device_ptrs.d_grad_MV));
            CL_CHECK(k_update_1st_h_2.setArg(7, device_ptrs.d_grad_MQ));
            CL_CHECK(k_update_1st_h_2.setArg(8, device_ptrs.d_grad_MK));
            CL_CHECK(k_update_1st_h_2.setArg(9, device_ptrs.d_grad_EH));
            CL_CHECK(k_update_1st_h_2.setArg(10, learning_rate));
            CL_CHECK(k_update_1st_h_2.setArg(11, cl_update_eh_flag_1sth_2));
            CL_CHECK(k_update_1st_h_2.setArg(12, lambda_l1));
            CL_CHECK(k_update_1st_h_2.setArg(13, lambda_l2));
            CL_CHECK(k_update_1st_h_2.setArg(14, host_max_grad_clip_value));
            CL_CHECK(k_update_1st_h_2.setArg(15, mat_heights));
            CL_CHECK(k_update_1st_h_2.setArg(16, embedding_dim));
            CL_CHECK(current_stream.enqueueNDRangeKernel(k_update_1st_h_2, cl::NullRange, global_proj_mat, local_1d));
    ...
    float& learning, float& lambda_l1, float& lambda_l2, float& clip_norm)
{
    ...
    const float learning_rate = learning;
    const float scaling_factor = SCALING;
    // Choose an appropriate value for your model (e.g., 1.0f, 5.0f, 10.0f)
    float host_max_grad_clip_value = MAX_GRAD_CLIP;
    ...
    try {
        ...
        for (int head_idx = num_heads_to_process - 1; head_idx >= 0; --head_idx) {
            ...
            cl::Kernel k_last_delta_h = clcontext.kernels.at("kernelLastLayerDeltaSigmoid"); 
            CL_CHECK(k_last_delta_h.setArg(0, device_ptrs.d_grad_EH)); // Now uses potentially clipped d_grad_EH
            CL_CHECK(k_last_delta_h.setArg(1, device_ptrs.d_hor_activations[num_neuron_layers_mlp - 1]));
            CL_CHECK(k_last_delta_h.setArg(2, device_ptrs.d_hor_deltas[num_neuron_layers_mlp - 1]));
            CL_CHECK(k_last_delta_h.setArg(3, embedding_dim));
            CL_CHECK(current_stream.enqueueNDRangeKernel(k_last_delta_h, cl::NullRange, global_embed, local_1d));
            ...
            CL_CHECK(k_grad_mlp_input.setArg(0, device_ptrs.d_ver_deltas[0]));
            CL_CHECK(k_grad_mlp_input.setArg(1, device_ptrs.d_ver_weights[0])); // Corrected potential typo from previous versions (should be `_ver_weights[0]`)
            CL_CHECK(k_grad_mlp_input.setArg(2, device_ptrs.d_grad_dv));
            CL_CHECK(current_stream.enqueueNDRangeKernel(k_grad_mlp_input, cl::NullRange, global_embed, local_1d));
            current_stream.finish(); // Ensure MLP input gradients are clipped.
            ...
            if (token_count > 0) {
                ...
                CL_CHECK(k_grad_mk_mq.setArg(8, embedding_dim));
                CL_CHECK(current_stream.enqueueNDRangeKernel(k_grad_mk_mq, cl::NullRange, global_matrix_2d, local_2d));
                current_stream.finish(); // Ensure attention gradients are clipped.
            ...
            CL_CHECK(k_update_weights_eh_ev.setArg(3, device_ptrs.d_MK_a));
            CL_CHECK(k_update_weights_eh_ev.setArg(4, device_ptrs.d_EH));
            CL_CHECK(k_update_weights_eh_ev.setArg(5, device_ptrs.d_EV));
            CL_CHECK(k_update_weights_eh_ev.setArg(6, device_ptrs.d_grad_MH)); // Clipped gradient used here
            CL_CHECK(k_update_weights_eh_ev.setArg(7, device_ptrs.d_grad_MV)); // Clipped gradient used here
            CL_CHECK(k_update_weights_eh_ev.setArg(8, device_ptrs.d_grad_MQ)); // Clipped gradient used here
            CL_CHECK(k_update_weights_eh_ev.setArg(9, device_ptrs.d_grad_MK)); // Clipped gradient used here
            CL_CHECK(k_update_weights_eh_ev.setArg(10, device_ptrs.d_grad_EH)); // Potentially clipped d_grad_EH used here
            CL_CHECK(k_update_weights_eh_ev.setArg(11, device_ptrs.d_grad_EV_scaled)); // Potentially clipped d_grad_EV_scaled used here
        ...
        // --- Calculate and Accumulate dL/dTokenEmbed for this column (after all heads in column processed) ---
        // This is done ONLY if it's the first column (layno_col_idx == 0)
        if(layno_col_idx == 0) {
            /*
            // cl::Kernel k_compute_grad_token_embed_for_head = clcontext.kernels.at("clComputeGradTokenEmbedForHead");
            cl::Kernel k_sum_matrices_elementwise = clcontext.kernels.at("clSumMatricesElementwise");
            // Loop through each head in this column to compute its dL/dTokenEmbed and accumulate
            for (int head_idx = 0; head_idx < num_heads_to_process; ++head_idx) {
                attention& head_obj = b[head_idx][layno_col_idx];
                HeadDeviceSubBuffersH& device_ptrs = head_gpu_data_cl[head_idx];
                cl::CommandQueue& current_stream = streams_cl[head_idx]; // Use the specific head's stream
                // 1. Compute dL/dTokenEmbed for THIS head's contribution
                //    Output is d_grad_token (which is device_ptrs.d_grad_token for this head)
                // Use the _lambda helper directly when setting args or creating NDRange for kernel launch
                cl::NDRange global_size_grad_token_head_2d_computed = calculate_global_2d(tokenCount, embedding_dim); // Assign result here
                CL_CHECK(k_compute_grad_token_embed_for_head.setArg(0, device_ptrs.d_grad_K)); // dL/dK
                CL_CHECK(k_compute_grad_token_embed_for_head.setArg(1, device_ptrs.d_grad_Q)); // dL/dQ
                CL_CHECK(k_compute_grad_token_embed_for_head.setArg(2, device_ptrs.d_MK_a));   // Original MK
                CL_CHECK(k_compute_grad_token_embed_for_head.setArg(3, device_ptrs.d_MQ_a));   // Original MQ
                CL_CHECK(k_compute_grad_token_embed_for_head.setArg(4, device_ptrs.d_grad_token)); // Output: dL/dTokenEmbed for THIS head
                CL_CHECK(k_compute_grad_token_embed_for_head.setArg(5, tokenCount));
                CL_CHECK(k_compute_grad_token_embed_for_head.setArg(6, mat_heights));
                CL_CHECK(k_compute_grad_token_embed_for_head.setArg(7, embedding_dim));
                // Use the correctly computed cl::NDRange variable for the kernel launch
                CL_CHECK(current_stream.enqueueNDRangeKernel(k_compute_grad_token_embed_for_head, cl::NullRange, global_size_grad_token_head_2d_computed, local_2d));
                CL_CHECK(current_stream.finish()); // Ensure calculation is done for this head
                // 2. Accumulate this head's dL/dTokenEmbed into the shared column output buffer
                int total_elements_token_embed_output = static_cast<int>(tokenCount) * embedding_dim;
                cl::NDRange global_size_sum_matrices_local = calculate_global_1d(total_elements_token_embed_output); // Corrected to use _lambda
                CL_CHECK(k_sum_matrices_elementwise.setArg(0, d_dL_dTokenEmbed_aggregated_column_output)); // Accumulator
                CL_CHECK(k_sum_matrices_elementwise.setArg(1, device_ptrs.d_grad_token));                  // Input from this head
                CL_CHECK(k_sum_matrices_elementwise.setArg(2, total_elements_token_embed_output));
                CL_CHECK(current_stream.enqueueNDRangeKernel(k_sum_matrices_elementwise, cl::NullRange, global_size_sum_matrices_local, local_1d));
                CL_CHECK(current_stream.finish()); // Ensure accumulation is done for this head
            }*/
