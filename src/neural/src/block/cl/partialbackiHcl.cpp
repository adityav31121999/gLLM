...
void block::clpartialbackward1stBlock(std::vector<std::vector<float>>& expectedH, int &in_dim, int &layers_mlp, int& layno_col_idx)
{
    ...
    try {
        ...
        for (int head_idx = num_heads_to_process - 1; head_idx >= 0; --head_idx) {
            ...
            for(int l=0; l<num_weight_matrices_mlp; ++l) { CL_CHECK(current_stream.enqueueReadBuffer(device_ptrs.d_hor_weights[l], CL_FALSE, 0, mlp_weights_bytes, head_obj.hor.weights[l].mapped_data)); CL_CHECK(current_stream.enqueueReadBuffer(device_ptrs.d_hor_gweights[l], CL_FALSE, 0, mlp_weights_bytes, head_obj.hor.gweights[l].mapped_data)); CL_CHECK(current_stream.enqueueReadBuffer(device_ptrs.d_ver_weights[l], CL_FALSE, 0, mlp_weights_bytes, head_obj.ver.weights[l].mapped_data)); CL_CHECK(current_stream.enqueueReadBuffer(device_ptrs.d_ver_gweights[l], CL_FALSE, 0, mlp_weights_bytes, head_obj.ver.gweights[l].mapped_data)); }
