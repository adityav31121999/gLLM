...
void block::cl1parallelForprop(int& in_dim_param, int& tokenCount_param, int col_idx_param, int& layers_mlp_param)
{
    ...
    size_t head_bytes = static_cast<size_t>(CONTEXT_WIN) * CONTEXT_WIN * sizeof(float);
    size_t sums_bytes = static_cast<size_t>(CONTEXT_WIN) * sizeof(float);
    size_t accum_bytes = static_cast<size_t>(CONTEXT_WIN) * sizeof(float);
    size_t proj_mat_bytes = static_cast<size_t>(EMBEDDING) * CONTEXT_WIN * sizeof(float);
    size_t ev_processed_bytes = static_cast<size_t>(CONTEXT_WIN) * EMBEDDING * sizeof(float);
    size_t embed_bytes = static_cast<size_t>(EMBEDDING) * sizeof(float);
    ...
    if (n_tokens > 0) {
        ...
        agg_d_EV_processed_data = cl::Buffer(clcontext.context, CL_MEM_READ_WRITE, num_heads_in_col * ev_processed_bytes, nullptr, &cl_err); CL_CHECK(cl_err);
    }
    ...
    cl::CommandQueue queue(clcontext.context, clcontext.device, 0, &cl_err);
    ...
    for (int layer_idx = 0; layer_idx < num_heads_in_col; ++layer_idx) {
        ...
        try {
            ...
            if (n_tokens > 0) {
                ...
                current_gpu_bufs.d_EV_processed_data = create_sub_buffer(agg_d_EV_processed_data, ev_processed_bytes); CL_CHECK(cl_err);
            }
            ...
            if (n_tokens > 0) {
                ...
                CL_CHECK(current_queue.enqueueWriteBuffer(current_gpu_bufs.d_EV_processed_data, CL_FALSE, 0, ev_processed_bytes, head_cpu.EV.mapped_data));
            }
            ...
            for (size_t layer_idx_mlp = 0; layer_idx_mlp < num_weight_matrices; ++layer_idx_mlp) {
                ...
                { 
                    ...
                    cl::Buffer d_mlp_weights(clcontext.context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, weights_bytes_mlp, current_weights_mat.mapped_data, &cl_err); CL_CHECK(cl_err); 
            ...
            if (n_tokens > 0) {
                CL_CHECK(current_queue.enqueueReadBuffer(current_gpu_bufs.d_EV_processed_data, CL_FALSE, 0, ev_processed_bytes, head_cpu.EV.mapped_data));
            }
