 ...
 * @param layers number of layers of MLP
 */
void block::partialforpropev(int& in, int& tokenCount, int i, int& layers) 
{
    // initialize the horizontal embedding vector to 0
    for (int j = 0; j < y; j++) {
        b[i][j].EH = std::vector<float>(in, 0.0f);
    ...
    // for one partial attention
    for(int j = 0; j < y; j++) {
        b[i][j].forpropev(in, layers, tokenCount);      // incomplete attention forprop
        // break when last head forprop is done
        if(j == (y - 1))
            break;
        b[i][j + 1].EH = b[i][j].EH;
 ...
 * @param n number of tokens for each head (context window)
 */
void block::partialforpropev(std::vector<std::vector<std::vector<float>>>& EVp_layer_from_prev_block, int& in, int& tokenCount, int& k, int current_block_layer_idx,
    int& layers, int& n)
{
    // initialize the horizontal embedding vector to 0
    for (int j = 0; j < y; j++) {
    ...
    for(int j = 0; j < y; j++) {
        ...
        b[current_block_layer_idx][j].forpropev(temp_EVp_mat, in, layers, tokenCount, k, n);      // incomplete attention forprop
        // break when last head forprop is done
        if(j == (y - 1))
            break;
        b[current_block_layer_idx][j + 1].EH = b[current_block_layer_idx][j].EH;
 ...
 * @param layers layers of mlp
 */
void block::forpropev(int& in, int& tokenCount, int& layers, int blockCount) 
{
    // deserialise(blockFilePath);
    // y partial attention in x layers => x parallel processes
    for(int i = 0; i < x; i++) {
        // for all layers
        partialforpropev(in, tokenCount, i, layers);
    }
}
 ...
 * @param blockCount current block position in full context
 */
void block::forpropev(std::vector<std::vector<std::vector<std::vector<float>>>>& EVp, int& in, int& tokenCount, int& blockCount, int& layers, 
                int& n)
{
    // deserialise(blockFilePath);
    // y partial attention in x layers => x parallel processes
    ...
    // forward propagation for all layers
    for(int i = 0; i < x; i++) {
        partialforpropev(EVp[i], in, tokenCount, blockCount, i, layers, n); // EVp[i] is the data for layer i of previous block
    }
}
