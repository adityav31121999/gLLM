...
public:
    ...
    void forpropev(int& in, int& layers, int& tokenCount);
    void forpropev(const mat& EVp, int& in, int& layers, int& tokenCount, int& blockCount, int& n);
    ...
    std::vector<float> backward1stHeadContext(std::vector<float>& expected, int& in, int& layers, int headnumber, float& learning);
    std::vector<float> backwardContext(std::vector<float>& expected, int& in, int& layers, int headnumber, float& learning);
...
// from kdotq.cu
    __global__ void kernelCompute_single_kq_vector(const float* d_token_embedding, const float* d_projection_matrix,
...
// forward propagation
    ...
    __global__ void KernelComputeGradDeEmbeddings(const float* d_delta, const float4* otok, float4* grad, int vocab, int dEmbedDim_div_4);
    __global__ void kernelGradForAttentionOutput(const float* d_deEmbed, const float* d_delta, float* grad4EH, int vocabSize, int dEmbedDim);
    __global__ void kernelComputeGradientsEH(const float* eh, const float* expected_h, float* grad_eh, int embedding_dim);
...
// update weights
    __global__ void kernelUpdateWeightsHeadHVElastic(float* mh_a, float* mv_a, float* mq_a, float* mk_a, float* eh, float* ev,
        const float* grad_mh, const float* grad_mv, const float* grad_mq, const float* grad_mk, const float* grad_eh, 
        const float* grad_ev_scaled, float learning_rate, int update_eh, int update_ev, float lambda_l1, float lambda_l2,
        float max_grad_clip_value, int mat_heights, int embedding_dim, int context_win);
    __global__ void kernelUpdateWeightsHeadElastic(float* mh_a, float* mv_a, float* mq_a, float* mk_a, float* eh, const float* grad_mh, 
        const float* grad_mv, const float* grad_mq, const float* grad_mk, const float* grad_eh, float learning_rate, int update_eh, 
        float lambda_l1, float lambda_l2, float max_grad_clip_value, int mat_heights, int embedding_dim);
    __global__ void kernelUpdateWeightsGeneral(float* weights, const float* gradients, float learning_rate, float lambda_l1, float lambda_l2,
        int total_elements);
    ...
    __global__ void kernelUpdateWeightsGeneral_f4(float* weights, const float* gradients, float learning_rate, float lambda_l1, 
        float lambda_l2, int total_elements);
    __global__ void updateEmbeddings(float* embeddings, const float* gradientsVector, float learning_rate, float lambda_l1, float lambda_l2,
        int embeddingRows, int embeddingDim);
#endif
